...
static void dlock(void) { qlock(&drawlock); }

static int candlock(void) { return canqlock(&drawlock); }

static void dunlock(void) { qunlock(&drawlock); }

static int drawgen(Chan *c, char *, Dirtab *, int, int s, Dir *dp) {
  int t;
  Qid q;
  ulong path;
  Client *cl;
...
static int drawrefactive(void *a) {
  Client *c;
...
static void drawrefresh(Memimage *, Rectangle r, void *v) {
  Refx *x;
  DImage *d;
  Client *c;
  Refresh *ref;
...
static void addflush(Rectangle r) {
  int abb, ar, anbb;
  Rectangle nbb;

  if (sdraw.softscreen == 0 || screenimage == nil ||
      !rectclip(&r, screenimage->r))
    return;
...
static void dstflush(int dstid, Memimage *dst, Rectangle r) {
  Memlayer *l;
...
DImage *drawlookup(Client *client, int id, int checkname) {
  DImage *d;
...
Memscreen *drawinstallscreen(Client *client, DScreen *d, int id, DImage *dimage,
...
void drawfreedscreen(DScreen *this) {
  DScreen *ds, *next;
...
void drawuninstallscreen(Client *client, CScreen *this) {
  CScreen *cs, *next;
...
Client *drawclientofpath(ulong path) {
  Client *cl;
  int slot;
...
Memimage *drawimage(Client *client, uchar *a) {
  DImage *d;
...
void drawrectangle(Rectangle *r, uchar *a) {
...
void drawpoint(Point *p, uchar *a) {
  p->x = BGLONG(a + 0 * 4);
  p->y = BGLONG(a + 1 * 4);
}

Point drawchar(Memimage *dst, Memimage *rdst, Point p, Memimage *src, Point *sp,
...
static DImage *makescreenimage(void) {
  int width, depth;
  ulong chan;
  DImage *di;
  Memdata *md;
  ...
  if (!waserror()) {
    snprint(screenname, sizeof screenname, "noborder.screen.%d",
            ++screennameid);
...
static int initscreenimage(void) {
  if (screenimage != nil)
    return 1;
...
void resetscreenimage(void) { initscreenimage(); }

static Chan *drawattach(char *spec) {
  dlock();
  if (!initscreenimage()) {
    dunlock();
...
static Walkqid *drawwalk(Chan *c, Chan *nc, char **name, int nname) {
  if (screenimage == nil)
    error("no frame buffer");
  return devwalk(c, nc, name, nname, 0, 0, drawgen);
}

static int drawstat(Chan *c, uchar *db, int n) {
...
/*@
  @ requires \valid(c);
  @ requires (n > 0 ==> \valid((char*)a + (0 .. (integer)n-1))) || (n == 0);
  @ assigns ((char*)a)[0 .. (integer)n-1] \if n > 0;
  @ ensures \result <= n;
  @*/
drawread(Chan *c, void *a, long n, vlong off)
{
  ...
  case Qctl:
    ...
    n = sprint(a, "%11d %11d %11s %11d %11d %11d %11d %11d %11d %11d %11d %11d",
               ...
               (i->flags & Frepl) == Frepl, i->r.min.x, i->r.min.y, i->r.max.x,
               i->r.max.y, i->clipr.min.x, i->clipr.min.y, i->clipr.max.x,
               i->clipr.max.y);
    ((char *)a)[n++] = ' ';
    cl->infoid = -1;
    break;
...
uchar *drawcoord(uchar *p, uchar *maxp, int oldx, int *newx) {
  int b, x;
...
static void printmesg(char *fmt, uchar *a, int plsprnt) {
  char buf[256];
  char *p, *q;
  int s;
  ...
  *q++ = *a++;
  for (p = fmt; *p; p++) {
    ...
    case 'L':
      ...
      break;
    case 'R':
      q += sprint(q, " [%d %d %d %d]", BGLONG(a), BGLONG(a + 4), BGLONG(a + 8),
                  BGLONG(a + 12));
      a += 16;
      break;
    case 'P':
...
void drawmesg(Client *client, void *av, int n) {
  int c, repl, m, y, dstid, scrnid, ni, ci, j, nw, e0, e1, op, ox, oy, oesize,
      esize, doflush;
  uchar *u, *a, refresh;
  char *fmt;
  ulong value, chan;
  ...
  while ((n -= m) > 0) {
    ...
    /* new allocate: 'b' id[4] screenid[4] refresh[1] chan[4] repl[1] R[4*4]
     * clipR[4*4] rrggbbaa[4] */
    case 'b':
    ...
    /* ellipse: 'e' dstid[4] srcid[4] center[2*4] a[4] b[4] thick[4] sp[2*4]
     * alpha[4] phi[4]*/
    case 'e':
    case 'E':
      ...
      dstflush(
          dstid, dst,
          Rect(p.x - e0 - j, p.y - e1 - j, p.x + e0 + j + 1, p.y + e1 + j + 1));
      continue;
    ...
    /* draw line: 'L' dstid[4] p0[2*4] p1[2*4] end0[4] end1[4] radius[4]
     * srcid[4] sp[2*4] */
    case 'L':
    ...
    /* filled polygon: 'P' dstid[4] n[2] wind[4] ignore[2*4] srcid[4] sp[2*4]
     * p0[2*4] dp[2*2*n] */
    /* polygon: 'p' dstid[4] n[2] end0[4] end1[4] radius[4] srcid[4] sp[2*4]
     * p0[2*4] dp[2*2*n] */
    case 'p':
    case 'P':
      ...
      if (pp == nil)
        error(Enomem);
      doflush = 0;
      if (dstid == 0 || (screenimage && dst->layer &&
                         dst->layer->screen->image->data == screenimage->data))
      ...
      for (y = 0; y < ni; y++) {
        ...
        if (doflush) {
          ...
          if (*a == 'P' && e0 != 1 && e0 != ~0)
            ...
            r = Rect(q.x - oesize, q.y - oesize, q.x + oesize + 1,
                     q.y + oesize + 1);
            combinerect(&r, Rect(p.x - esize, p.y - esize, p.x + esize + 1,
                                 p.y + esize + 1));
          }
          if (rectclip(&r,
                       dst->clipr)) /* should perhaps be an arg to dstflush */
            dstflush(dstid, dst, r);
        }
        pp[y] = p;
      }
      if (y == 1)
        dstflush(
            dstid, dst,
            Rect(p.x - esize, p.y - esize, p.x + esize + 1, p.y + esize + 1));
      op = drawclientop(client);
      if (*a == 'p')
    ...
    /* string: 's' dstid[4] srcid[4] fontid[4] P[2*4] clipr[4*4] sp[2*4] ni[2]
     * ni*(index[2]) */
    /* stringbg: 'x' dstid[4] srcid[4] fontid[4] P[2*4] clipr[4*4] sp[2*4] ni[2]
     * bgid[4] bgpt[2*4] ni*(index[2]) */
    case 's':
    case 'x':
    ...
    for (v = 0; v != 4; v++, i += 16) {
      for (g = 0, j = v - r; g != 4; g++)
        for (b = 0; b != 4; b++, j++) {
          ...
          setcolor(i + (j & 15), cr * 0x01010101, cg * 0x01010101,
                   cb * 0x01010101);
        }
    }
}
