...
static void *resolve_kernel_symbol(char *name) {
  ...
  /* Direct mapping for IL resolution */
  if (strcmp(name, "clr_console_write") == 0)
    return (void *)clr_console_write;
  if (strcmp(name, "clr_console_writeline") == 0)
    return (void *)clr_console_writeline;

  /* F# Printf - direct kernel print (bypasses cross-assembly resolution) */
  if (strcmp(name, "Microsoft_FSharp_Core_Printf_Internal_PrintLine") == 0)
    return (void *)clr_console_writeline;
  if (strcmp(name, "Microsoft_FSharp_Core_Printf_Internal_Print") == 0)
    return (void *)clr_console_write;

  /* Lux9.Init KernelPrint - direct kernel print for init process */
  if (strcmp(name, "Lux9_Init_KernelPrint_Internal_PrintLine") == 0)
    return (void *)clr_console_writeline;
  if (strcmp(name, "Lux9_Init_KernelPrint_Internal_Print") == 0)
    return (void *)clr_console_write;
...
int qbe_compile_page(uintptr qbe_page, uintptr asm_page, char *errorbuf,
                     usize errorbuf_size) {
  void *qbe_vaddr = (void *)qbe_page;
  void *asm_vaddr = (void *)asm_page;
  u8int *code = (u8int *)asm_page;
  char *p;
  char token[64];
  extern void uartputs(char *, int);
  char debug_buf[128];
  ...
  /* DEBUG: Enable JIT debug for a while */
  jitdebug = 1;
