...
/* Forward decls */
static int familygen(Chan *c, char *name, Dirtab *tab, int ntab, int pos,
                     Dir *dp);

/*@
  @ assigns \result; // Actually void, but assigns global family registry.
  @ // Better: assigns family_registry;
  @*/
static void familyinit(void) { family_init(); }
...
/*@
  ...
  @ terminates \true;
  @*/
static Chan *familyattach(char *spec) { return devattach('F', spec); }
...
/*@
  ...
  @ ensures \result == \null || \valid(\result);
  @*/
static Walkqid *familywalk(Chan *c, Chan *nc, char **name, int nname) {
...
/*@
  ...
  @*/
static int familystat(Chan *c, uchar *dp, int n) {
  int type = (c->qid.path & FAMILY_MASK) >> FAMILY_SHIFT;
...
/*@
  ...
  @ ensures \result == \null || \result == c;
  @*/
static Chan *familyopen(Chan *c, int omode) {
  int type = (c->qid.path & FAMILY_MASK) >> FAMILY_SHIFT;
...
/*@
  ...
  @ assigns *family;
  @*/
static void familyclose(Chan *c) {
  int type = (c->qid.path & FAMILY_MASK) >> FAMILY_SHIFT;
...
/*@
  ...
  @ ensures \result == -1 || \result >= 0;
  @*/
static long familyread(Chan *c, void *va, long n, vlong off) {
  int type = (c->qid.path & FAMILY_MASK) >> FAMILY_SHIFT;
...
/*@
  ...
  @ ensures \result == -1 || \result >= 0;
  @*/
static long familywrite(Chan *c, void *va, long n, vlong off) {
  int type = (c->qid.path & FAMILY_MASK) >> FAMILY_SHIFT;
...
Dev familydevtab = {
    ...
    devreset,   familyinit,  devshutdown, familyattach, familywalk,
    familystat, familyopen,  devcreate,   familyclose,  familyread,
    devbread,   familywrite, devbwrite,   devremove,    devwstat,
};