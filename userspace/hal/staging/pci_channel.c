...
/* Note: PCIChannel and PCIChannelManager structures are defined in
 * pci_family_ops.h */
...
struct PCIEChannelManager {
  ...
  int (*allocate_by_address)(struct PCIDeviceDescriptor *dev,
                             uint64_t *channel_id);
  int (*allocate_by_name)(char *device_name, uint64_t *channel_id);
  int (*allocate_auto)(struct PCIDeviceDescriptor *dev, void *criteria,
                       uint64_t *channel_id);
};
...
static struct PCIDeviceDescriptor *pci_discover_device(uint8_t bus, uint8_t dev,
                                                       uint8_t func) {
  return nil;
}
static int validate_channel_operation_permission(Proc *proc,
                                                 struct PCIChannel *ch) {
  return 0;
}
/*@
  @ requires \valid(res);
...
/*@
  @ assigns \nothing;
  @*/
static uint32_t get_max_channels_for_memory(uint32_t max_channels) {
  return MAX_CHANNELS_PER_PCI_FAMILY;
}
/*@
  @ assigns \nothing;
  @*/
static uint32_t get_max_channels_for_device_count(uint32_t device_count) {
  return device_count * 4;
}
static int min(int a, int b) { return a < b ? a : b; }
static int exchange_prepare_pages(uintptr vaddr, size_t size,
                                  ExchangeHandle *handle, int prot) {
...
/* static void lock_init(Lock* l) { memset(l, 0, sizeof(Lock)); } - Moved to
 * stubs.c */
static Proc *current_process(void) { return up; }

/* Helper function to convert channel state to string */
static const char *channel_state_to_string(enum ChannelState state) {
  switch (state) {
...
struct PCIChannel *allocate_pci_channel_struct(
  ...
  if (mgr->free_channels) {
    ...
    channel = xalloc_driver(sizeof(struct PCIChannel));
    if (!channel) {
      unlock(&mgr->channel_lock);
      return NULL;
    }
  ...
  snprint(channel->channel_name, sizeof(channel->channel_name),
          "pci_%04x_%02x_%02x.%x_ch%lu", device->address.domain,
...
static int validate_pci_channel_permissions(uint32_t req_perms,
  ...
  if (dev->device_state == DEVICE_ERROR ||
      dev->device_state == DEVICE_REMOVED) {
...
static int pci_allocate_channel_by_address(struct FamilyExchangePage *family,
                                           void *device_id,
...
static int pci_allocate_channel_by_name(struct FamilyExchangePage *family,
                                        char *device_name, uint32_t permissions,
                                        uint64_t *channel_id) {
  struct PCIEChannelManager *mgr =
      (struct PCIEChannelManager *)family->channel_mgr;
...
static int pci_allocate_channel_auto(struct FamilyExchangePage *family,
                                     void *device_criteria,
                                     uint32_t permissions,
                                     uint64_t *channel_id) {
  struct PCIDeviceCriteria *criteria =
      (struct PCIDeviceCriteria *)device_criteria;
  struct PCIEChannelManager *mgr =
      (struct PCIEChannelManager *)family->channel_mgr;
  ...
  @*/
    ...
    if (criteria->class_code != 0xFF &&
        dev->class_code == criteria->class_code) {
      score += criteria->class_match_weight;
    }

    /* Check vendor/device match */
    if (criteria->vendor_id != 0xFFFF &&
        dev->vendor_id == criteria->vendor_id) {
      score += criteria->vendor_match_weight;
    }
    if (criteria->device_id != 0xFFFF &&
        dev->device_id == criteria->device_id) {
  ...
  uint64_t new_channel_id = generate_pci_channel_id();
  struct PCIChannel *channel = allocate_pci_channel_struct(
      family, new_channel_id, best_device, permissions);
  if (!channel) {
...
void setup_pci_channel_manager(struct FamilyExchangePage *family) {
  ...
  mgr->channels = xalloc_driver(sizeof(struct PCIChannel) * mgr->max_channels);
  mgr->channel_ids =
      xalloc_driver(sizeof(struct PCIChannel *) * mgr->max_channels);

  if (!mgr->channels || !mgr->channel_ids) {
    if (mgr->channels)
      xfree_driver(mgr->channels);
    if (mgr->channel_ids)
      xfree_driver(mgr->channel_ids);
    xfree_driver(mgr);
    return;
  }
...
static int pci_configure_channel_limits(struct FamilyExchangePage *family) {
  ...
  uint32_t max_dev_channels = get_max_channels_for_device_count(
      mgr->family_ctx->topology.total_devices);
...
static int pci_channel_allocate_bar_resource(struct PCIChannel *channel,
                                             uint8_t bar_num, uint32_t size,
                                             uintptr *virtual_addr) {
  ...
  /* Record allocation */
  if (!bar_res) {
    bar_res = xalloc_driver(sizeof(struct PCIChannelBarResource));
    if (!bar_res) {
      exchange_cancel(&bar_handle);
      return -7;
    }
  ...
  print("PCI: channel %d BAR%d mapped at %p (size=%d)\n", channel->channel_id,
        bar_num, virt_addr, size);

  return 0;
}

/* Unmap and release BAR resource */
static int pci_channel_release_bar_resource(struct PCIChannel *channel,
                                            uint8_t bar_num) {
  ...
  xfree_driver(bar_res);

  return 0;
}

/* Get BAR resource info */
static int pci_channel_get_bar_info(struct PCIChannel *channel, uint8_t bar_num,
                                    void *info_buffer, size_t *info_size) {
  if (!channel || !channel->bound_device || bar_num >= 6) {
    return -1;
  }
...
void cleanup_pci_channel_resources(struct PCIChannel *channel) {
  ...
  for (int i = 0; i < 6; i++) {
    if (channel->resources.bars[i]) {
      ...
      xfree_driver(channel->resources.bars[i]);
  ...
  for (int i = 0; i < 8; i++) {
    if (channel->resources.irqs[i]) {
      ...
      xfree_driver(channel->resources.irqs[i]);
  ...
  for (int i = 0; i < 4; i++) {
    if (channel->resources.dmas[i]) {
      ...
      xfree_driver(channel->resources.dmas[i]);
