...
static void pcirouting(void) {
  ...
  if ((p = sigsearch("$PIR", 0)) == nil) {
    print("pcirouting: $PIR not found\n");
    return;
  }
  ...
  if (size < sizeof(Router) || checksum(r, size)) {
    print("pcirouting: bad $PIR table\n");
    return;
  }
  ...
  if (sbpci == nil) {
    print("pcirouting: Cannot find south bridge tbdf=%ux\n", tbdf);
    return;
  }
  ...
  if (i == nelem(southbridges)) {
    print("pcirouting: ignoring south bridge tbdf=%ux vid=%ux did=%ux\n", tbdf,
          sbpci->vid, sbpci->did);
    return;
  }
  southbridge = &southbridges[i];
  if (southbridge->get == nil) {
    print("pcirouting: southbridge->get is nil, returning\n");
    return;
  }

  pciirqs = (r->pciirqs[1] << 8) | r->pciirqs[0];
  print("pcirouting: pciirqs=0x%ux starting slot loop\n", pciirqs);
  for (e = (Slot *)&r[1]; (uchar *)e < p + size; e++) {
    print("pcirouting: slot bus=%d dev=%d\n", e->bus, e->dev);
    if (0) {
...
void pcicfginit(void) {
  ...
  print("pcicfginit: mode detection start\n");
  ...
  print("pcicfginit: pcimaxbno=%d pcimaxdno=%d\n", pcimaxbno, pcimaxdno);

  list = &pciroot;
  for (bno = 0; bno <= pcimaxbno; bno++) {
    int sbno = bno;
    print("pcicfginit: scanning bus %d\n", bno);
  ...
  print("pcicfginit: pcireserve start\n");
  pcireserve();

  if (!nopcirouting) {
    print("pcicfginit: pcirouting start\n");
    pcirouting();
  }
  print("pcicfginit: done\n");
