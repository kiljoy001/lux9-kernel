...
static int is_leaf(ArtNode *n) {
    return n == nil; /* Simplified: Leaves are stored in parent pointers differently? 
...
static void* art_search(ArtNode *n, uchar *key, int len, int depth) {
    ...
    if (child && *child) {
        ...
        if (depth == len - 1) {
            ...
            if ((*child)->type == 0) return ((ArtLeaf*)*child)->value;
...
static SophiaNode* edge_lookup(SophiaNode *parent, char *name) {
    ...
    if (klen + nlen > EDGE_KEY_MAX) return nil;
    ...
    if (r->ifcall.nwname > 0) {
        for (i = 0; i < r->ifcall.nwname; i++) {
            ...
            if (!next) {
                ...
                if (i == 0) srv_respond(r, "file not found");
                else {
                    /* Partial success */
                    r->ofcall.nwqid = i;
                    srv_respond(r, nil);
        ...
        if (r->newfid) r->newfid->aux = curr;
        else r->fid->aux = curr; /* Should not happen in standard 9P walk usually implies clone */
    }
