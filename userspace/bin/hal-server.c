...
#include <libc.h>
#include <fcall.h>
#include "../../kernel/include/exchange.h"
...
hal_tpm_init(void)
{
	...
	/* TODO: Call kernel family TPM interface */
}

/* Scan Hardware (Stub) */
void
hal_scan_hardware(void)
{
	print("HAL: Scanning hardware via Family subsystem...\n");
	/* TODO: Walk /dev/family/pci */
}

/* Initialize Exchange Pages for 9P Transport */
void
hal_exchange_init(void)
{
	void *rx_mem, *tx_mem;
	int ret;
	...
	if(ret < 0) sysfatal("HAL: Failed to allocate RX ring via pebble");
	
	ret = pebble_black_alloc(sizeof(ExchangeRing), &tx_mem);
	if(ret < 0) sysfatal("HAL: Failed to allocate TX ring via pebble");
...
rx_ring->size = RING_SIZE;
	...
	if(rx_handle == 0) sysfatal("HAL: Failed to prepare RX exchange page");
	
	tx_handle = exchange_prepare((uintptr)tx_ring);
	if(tx_handle == 0) sysfatal("HAL: Failed to prepare TX exchange page");
...
/* Ring Buffer Read */
int
ring_read(ExchangeRing *ring, u8int *buf, int len)
{
	int available;
	int head, tail;
	int n1, n2;
...
/* Ring Buffer Write */
int
ring_write(ExchangeRing *ring, u8int *buf, int len)
{
	int free_space;
	int head, tail;
	int n1, n2;
...
void
hal_process_9p(u8int *msg, int len)
{
	u32int size;
	u8int type;
	u16int tag;
	...
	case Tversion:
		...
		/* msize = 8192 */
		reply[7] = 0x00; reply[8] = 0x20; reply[9] = 0x00; reply[10] = 0x00;
		
		/* version = "9P2000" */
		reply[11] = 0x06; reply[12] = 0x00; /* len */
