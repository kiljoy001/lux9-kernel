 ...
 *   - Tags uniquely allocated (no_double_alloc)
 *   - Alloc/free are inverses (alloc_free_inverse)
 *   - RPC counts balanced (ralloc_free_inverse_used)
 */
...
int mntversion(Chan *c, char *version, int msize, int returnlen) {
  ...
  eqlock(&c->umqlock); /* make sure no one else does this until we've
  ...
  l = devtab[devno(c->type, 0)]->write(c, msg, k, oo);
  if (l < k) {
    lock(&c->lock);
    c->offset -= k - l;
    unlock(&c->lock);
  ...
  for (k = 0; k < BIT32SZ || (k < GBIT32(msg) && k < MAXRPC0); k += l) {
    l = devtab[devno(c->type, 0)]->read(c, msg + k, MAXRPC0 - k, c->offset);
    if (l <= 0)
...
static Chan *noattach(char *) { error(Enoattach); }
...
static Chan *mntopencreate(int type, Chan *c, char *name, int omode,
                           ulong perm) {
  Mnt *m;
  Mntrpc *r;
...
static Chan *mntcreate(Chan *c, char *name, int omode, ulong perm) {
  return mntopencreate(Tcreate, c, name, omode, perm);
}

static void mntclunk(Chan *c, int t) {
  Mnt *m;
  Mntrpc *r;
...
static void mntclose(Chan *c) { mntclunk(c, Tclunk); }

static void mntremove(Chan *c) { mntclunk(c, Tremove); }
...
static long mntrdwr(int type, Chan *c, void *buf, long n, vlong off) {
  Mnt *m;
  Mntrpc *r;
  char *uba;
  ulong cnt, nr, nreq;
...
static int mntprocwork(void *a) {
  Mntproc *p = a;
  return p->f != nil;
}

static void mntproc(void *a) {
  Mntproc *p = a;
  Chan *c;
  Mnt *m;
...
static int mntdefer(void (*f)(Mntrpc *, void *), Mntrpc *r, void *a) {
  Mntproc *p;
  Mnt *m;
  int i;
...
static void rahproc(Mntrpc *r, void *a) {
  Mntrah *rah = a;
...
static int rahdone(void *v) {
  Mntrpc *r = v;
  return r->done == 2;
}

static Mntrpc *rahfindrpc(Mntrah *rah, vlong off) {
  Mntrpc *r;
  int i, n;
  vlong o;
...
static void mountrpc(Mnt *m, Mntrpc *r) {
  ...
  default:
    ...
    print("mnt: proc %s %lud: mismatch from %s %s rep %#p tag %d fid %d T%d "
          "R%d rp %d\n",
          up->text, up->pid, chanpath(m->c), chanpath(r->c), r, r->request.tag,
          r->request.fid, r->request.type, r->reply.type, r->reply.tag);
    error(Emountrpc);
  }
}

static void mountio(Mnt *m, Mntrpc *r) {
  Block *b;
  int n;
  ...
  if (n <= 0 || n > m->msize) {
    print("mountio: proc %s %lud: convS2M returned %d for tag %d fid %d T%d\n",
          up->text, up->pid, n, r->request.tag, r->request.fid,
          r->request.type);
    error(Emountrpc);
  }
  b->wp += n;
  if (r->request.type == Twrite && cachedchan(r->c))
    r->w = copyblock(b, n);
  poperror();
  devtab[devno(m->c->type, 0)]->bwrite(m->c, b, 0);
...
static int doread(Mnt *m, int len) {
  ...
  while (qlen(m->q) < len) {
    b = devtab[devno(m->c->type, 0)]->bread(m->c, m->msize, 0);
...
static int mntrpcread(Mnt *m, Mntrpc *r) {
  int i, t, len, hlen;
  Block *b, **l, *nb;
 ...
 * requests from it
 */
static Mntrpc *mntflushalloc(Mntrpc *r) {
  Mntrpc *fr;
 ...
 */
static Mntrpc *mntflushfree(Mnt *m, Mntrpc *r) {
  Mntrpc *fr;
...
/*@ requires t >= 0 && t < 65535;
    ensures !(mntalloc.tagmask[t >> TAGSHIFT] & (1 << (t & TAGMASK)));
...
/*
 * CRITICAL BUG (VERIFIED): tag reuse race existed here
 *
 * FORMAL SPECIFICATION (ACSL):
 ...
 *
 * FIX:
 *   Remove queued RPCs before the tag can be recycled.
...
/*@ requires \valid(r);
  ...
  @ behavior freed:
  @   assumes mntalloc.nrpcfree >= 32;
  @   // Tags are only freed after queue removal.
  @   ensures mntalloc.nrpcused == \old(mntalloc.nrpcused) - 1;
  @
  @ complete behaviors;
  @ disjoint behaviors;
 ...
 */
static void mntdirfix(uchar *dirbuf, Chan *c) {
  uint r;

  r = devtab[devno(c->type, 0)]->dc;
...
Dev mntdevtab = {
    ...
    mntreset, devinit,  devshutdown, noattach,  mntwalk,
