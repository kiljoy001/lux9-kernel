...
static void add_white_ref(clr_object_t *obj, clr_white_ref_t *ref) {
  lock(&obj->lock);
...
static void remove_white_ref(clr_object_t *obj, clr_white_ref_t *ref) {
  lock(&obj->lock);
...
static void heap_add_object(clr_heap_t *heap, clr_object_t *obj) {
...
static void heap_remove_object(clr_heap_t *heap, clr_object_t *obj) {
  lock(&heap->objects_lock);
...
clr_object_t *clr_object_alloc(clr_heap_t *heap, ulong size,
  ...
  white =
      pebble_issue_white(heap->pebble, (void *)entry.physical_address, size);
  if (white == nil) {
    pebble_black_free(&obj->black_cap);
    free(obj);
...
int clr_object_release(clr_heap_t *heap, clr_object_t *obj,
                       PebbleWhite *white) {
...
int clr_object_snapshot(clr_heap_t *heap, clr_object_t *obj) {
  ...
  if (obj->red_snapshot != nil) {
    unlock(&obj->lock);
    return 0; /* Idempotent */
  }

  /* Allocate Red token (COLORLESS -> RED) */
  obj->red_snapshot = pebble_red_alloc(obj->size);
  if (obj->red_snapshot == nil) {
    unlock(&obj->lock);
    error(PEBBLE_E_NOMEM);
  }

  /* Copy current state (Black) to Red snapshot */
  memmove(obj->red_snapshot->red_data, obj->data, obj->size);
...
int clr_object_commit(clr_heap_t *heap, clr_object_t *obj) {
  ...
  /* If no snapshot, nothing to commit */
  if (obj->red_snapshot == nil) {
    unlock(&obj->lock);
    return 0;
  }

  /* Commit means we accept current state. Free the snapshot (Red). */
  /* State transition: RED -> COLORLESS */
  pebble_red_free(obj->red_snapshot);
  obj->red_snapshot = nil;
...
int clr_object_rollback(clr_heap_t *heap, clr_object_t *obj) {
  ...
  /* If no snapshot, nothing to rollback */
  if (obj->red_snapshot == nil) {
    unlock(&obj->lock);
    return 0;
  }

  /* Restore state from snapshot (Red -> Black) */
  memmove(obj->data, obj->red_snapshot->red_data, obj->size);

  /* Free the snapshot (Red -> COLORLESS) */
  pebble_red_free(obj->red_snapshot);
  obj->red_snapshot = nil;
...
clr_result_t clr_pebble_execute(clr_pebble_state_t *state,
  ...
  case CIL_LDLOC:
    ...
    if (clr_local_load(state->heap, state->locals, instruction->arg.index_arg,
                       &v1, &obj1) != 0)
      return CLR_ERROR_INVALID_LOCAL;
