...
ahciidentify(Aportc *pc, ushort *id, uint *ss, char *d)
{
	...
	s = idfeat(m, id);
	*ss = idss(m, id);
	if(s == -1 || (m->sfis.feat&Dlba) == 0){
		if((m->sfis.feat&Dlba) == 0)
...
ahcirecover(Aportc *pc)
{
	ahciswreset(pc);
	pc->p->cmd |= Ast;
	if(settxmode(pc, pc->m->sfis.udma) == -1)
		return -1;
	return 0;
}
...
ignoreahdrs(Drive *d)
{
	return d->portm.sfis.feat & Datapi && d->ctlr->type == Tsb600;
}
...
newdrive(Drive *d)
{
	...
	if(settxmode(c, m->sfis.udma) == -1){
		dprint("%s: can't set udma mode\n", dnam(d));
		goto lose;
	}
	if(m->sfis.feat & Dpower && setfeatures(c, 0x85, 3*1000) == -1){
...
blink(Drive *d, ulong t)
{
	...
	case Elmt:
		...
		msg.led[0] = d->ledport.ledbits;
		msg.led[1] = d->ledport.ledbits>>8;
		msg.pm = 0;
		msg.hba = d->driveno;
		memmove(c->aenc.enctx, &msg, sizeof msg);
		break;
	}
	h->emctl |= Tmsg;
	return 1;
...
blinkesb(Ctlr *c, ulong t)
{
	...
	for(i = 0; i < c->ndrive; i++){
		d = c->drive[i];
		s = Esbdrv0 + Esbiota*i;
		v = esbbits(d->ledport.ledbits) * (1ull << s%32);
		u[s/32 + 0] |= v;
		u[s/32 + 1] |= v>>32;
	}
	for(i = 0; i < c->aenc.encsz; i++)
		c->aenc.enctx[i] = u[i];
	return 1;
}
...
ledkproc(void*)
{
	...
	for(i = 0; i < niadrive; i++){
		d = iadrive[i];
		d->ledport.nled = 3;		if(d->ctlr->enctype == Eesb)
			d->ledport.nled = 3;
		d->ledport.ledbits = -1;
	}
	for(i = 0; ; i++){
...
{
	c->aenc.encsz = 1;
	c->aenc.enctx = (ulong*)(c->mmio + 0xa0);
	c->enctype = Eesb;
	c->aenc.enctx[0] = 0;
	return 0;
}
...
iarctl(SDunit *u, char *p, char *e)
{
	...
	p = seprint(p, e, "alignment %d %d\n", 
		d->secsize<<d->portm.sfis.physshift, d->portm.sfis.physalign);
