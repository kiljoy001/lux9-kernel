(*
 * MSGORD Complete Proofs - NO ADMITS
...
Inductive LockType := IPC_LOCK | MSGORD_LOCK | PORT_LOCK.
...
(* COMPLETE PROOF: MSGORD Complexity Bound *)
Theorem msgord_complexity_bounded :
...
(* COMPLETE PROOF: MSGORD Provides Total Ordering *)
Theorem msgord_total_ordering :
...
Theorem msgord_correctness_complete :
...
Proof.
  ...
  - (* Memory safety *)
    ...
    exact (msgord_memory_safety resources new_resource H_mem_valid H_disjoint).
    
  - (* Performance bound *)
    apply msgord_complexity_bounded; assumption.
    
  - (* Resource bound *)
    apply msgord_memory_bounded with (dag_nodes := dag_nodes); [assumption | reflexivity].
    
  - (* Total ordering *)
    destruct (msgord_total_ordering dag_nodes H_dag_bound) as [order [H_total H_applies]].
    exists order.
    exact H_total.
Qed.