...
#define settxtflush(p, c)                                                      \
  if (c)                                                                       \
  memset((p)->txtflush, ~0, sizeof((p)->txtflush))
#define needtxtflush(p)                                                        \
  ((p)->txtflush[m->machno >> 5] & (1 << (m->machno & 0x1F)))
#define donetxtflush(p)                                                        \
  ((p)->txtflush[m->machno >> 5] &= ~(1 << (m->machno & 0x1F)))
#endif
...
#define PGHASH(i, daddr)                                                       \
  ((i)->pghash[((daddr) >> PGSHIFT) & ((i)->pghsize - 1)])
