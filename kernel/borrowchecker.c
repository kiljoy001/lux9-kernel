...
static int borrow_fsm_transition(struct BorrowOwner *owner,
  ...
  case BORROW_FREE:
    ...
    break;
  case BORROW_EXCLUSIVE:
    if (next == BORROW_FREE && owner->shared_count == 0 &&
        owner->mut_borrower == nil) {
      owner->state = next;
      return 1;
    }
    ...
    if (next == BORROW_MUT_LENT && owner->shared_count == 0 &&
        owner->mut_borrower == nil) {
      owner->state = next;
      return 1;
    }
    ...
    break;
  case BORROW_SHARED_OWNED:
    if (next == BORROW_EXCLUSIVE && owner->shared_count == 0 &&
        owner->mut_borrower == nil) {
      owner->state = next;
      return 1;
    }
...
/**
 * Initialize the global borrow pool and allocate its bucket table using the
 * bootstrap allocator.
 *
 * Sets the pool to 1024 buckets, allocates the bucket array via
 ...
 *
 * The new BorrowOwner is allocated using the bootstrap allocator, initialized
 * with default fields (no owner, state BORROW_FREE, system_owner
 * OWNER_BOOTLOADER, is_system_owned == 0, zeroed counters and lists), and
 * prepended into the borrow pool bucket computed from `key`.
 *
 ...
 *
 * Validates that `p` is the current owner and that there are no active shared
 * or mutable borrows; on success clears ownership, marks the resource free,
 * removes the owner entry from the hash table, and frees that entry only if
 * `xinit_done` indicates post-initialization.
 *
 * @param p Process attempting the release; must be non-nil and the current
 * owner.
...
enum BorrowError borrow_release(Proc *p, uintptr key) {
  ...
  prev = nil;
  for (owner = borrowpool.owners[hash].head; owner != nil;
       owner = owner->next) {
    if (owner->key == key) {
      if (prev == nil) {
 ...
 *
 * Adds the borrower to the owner's shared-borrow list and updates
 * owner/resource state and global counters when the operation succeeds.
 *
 ...
 * @returns BORROW_ENOTFOUND if no owner entry exists for `key`.
 * @returns BORROW_ENOTOWNER if `owner` is not the recorded owner of the
 * resource.
 * @returns BORROW_EMUTBORROW if a mutable borrow is active for the resource.
 * @returns BORROW_EALREADY if `borrower` already holds a shared borrow for the
 * resource.
 ...
 *
 * Scans the global borrow pool and: force-releases resources owned by the
 * process, clears shared and mutable borrows held by the process, removes the
 * process from other owners' shared lists, and removes owner entries that
 * become unused.
 *
 * Memory allocated for SharedBorrower and BorrowOwner nodes is freed when the
 * global xinit_done flag indicates full initialization; otherwise nodes are
 * only detached but not freed.
 *
 ...
 * If ownership is recorded after system initialization completes, records the
 * acquisition timestamp; before initialization the timestamp is set to zero.
 *
 * @param key Identifier for the resource (e.g., physical page frame or resource
 * key).
 ...
 *
 * Clears the resource's system ownership and marks it free; if removal
 * conditions are met the owner entry is also removed from the pool. Fails if
 * the resource does not exist, is not owned by `owner`, or has active shared or
 * mutable borrows.
 *
 * @param key Resource identifier (key) whose system ownership should be
 * released.
 * @param owner System owner expected to currently hold ownership for the
 * resource.
 * @returns BORROW_OK on success.
 * @returns BORROW_ENOTFOUND if no owner entry exists for `key`.
...
/**
 * Transfer system-level ownership of a resource identified by `key` from one
 * system owner to another.
 *
 * Updates the resource's system owner and records a new acquisition timestamp
 * when the system is initialized; if the system is not yet initialized
 * (`xinit_done` is false) the timestamp is set to 0.
 *
 * @param from The current system owner expected to hold ownership of the
 * resource.
 * @param to The system owner to transfer ownership to.
 * @param key The resource identifier whose system ownership will be
 * transferred.
 * @returns BORROW_OK on success.
 ...
 * with a system owner.
 *
 * The range is identified by the provided start and end physical addresses and
 * will be recorded in the global memory range list. Allocation for the internal
 * tracking entry is performed automatically: before xinit completes a bootstrap
 * allocation is used, and after xinit the entry is allocated with the regular
 * allocator.
 *
...
/**
 * Add a memory ownership range to the global list, allocating the tracking
 * entry.
 *
 ...
 *
 * @param start Starting address of the range to remove (same value used when
 * the range was added).
 ...
 *
 * Before the allocator initialization completes (xinit not done), this returns
 * a conservative capacity based on the bootstrap allocator. After
 * initialization it returns a very large value representing essentially
 * unlimited capacity.
 *
...
/**
 * Acquire system ownership for a physical address range, performed
 * page-by-page.
 *
 ...
 *          BORROW_EALREADY if a page was already owned (treated as success for
 * that page). Other BorrowError values if acquisition fails for a page; in that
 * case previously acquired pages are released and the error is returned.
 */
...
/**
 * Determine whether every page in a physical address range is owned by a given
 * system owner.
 *
 * Checks ownership per 4KB page across the range [start_pa, start_pa + size).
 * Before kernel initialization (when xinit_done is false) the function consults
 * the bootstrap memory range tracking; after initialization it checks runtime
 * per-page system ownership.
 *
 * @param start_pa Starting physical address of the range (inclusive).
 * @param size Size of the range in bytes; the check is performed per 4KB page
 * over the range.
 * @param owner System owner to verify for each page in the range.
 * @returns `1` if every page in the specified range is owned by `owner`, `0`
 * otherwise.
 */
 ...
 *
 * Checks access at 4KB page granularity. Before full kernel initialization
 * (when xinit_done is false) the function consults the boot-time memory range
 * tracker; after initialization it verifies per-page system ownership.
 *
...
/**
 * Discover runtime memory regions and register kernel-owned ranges with the
 * memory tracker.
 *
 * Scans the global configuration's memory table and registers regions that
 * belong to the kernel with the memory-range tracking subsystem; prints
 * discovered regions and then dumps the current range table.
 *
 * Note: If invoked before xinit has completed, the function logs an error and
 * returns without registering any ranges.
 */
