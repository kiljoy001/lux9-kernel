...
static Rune cp437[256] = {
    0x2007, 0x263A, 0x263B, 0x2665, 0x2666, 0x2663, 0x2660, 0x2022, 0x25D8,
...
void screeninit(void) {
   ...
   * 1. fbconsoleinit() - Framebuffer console (primary)
   * 2. uart_screenputs - Serial UART (always available)
   *
   * CGA required direct VGA memory access at 0xB8000 which
   * may not be mapped in UEFI/Limine boot environments.
   * Framebuffer console is already initialized before this
   * function is called, so we preserve that by doing nothing.
   */
  return;
}
