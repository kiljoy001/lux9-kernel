 ...
 */
static void intrtime(Mach *, int vno) {
  ulong diff;
  ulong x;
...
int irqhandled(Ureg *ureg, int vno) {
  Vctl *ctl, *v;
  int i;
  ...
  if (*pv != nil) {
    if ((*pv)->isr != v->isr || (*pv)->eoi != v->eoi) {
      print("intrenable: incompatible handler: %s %s %#p %#p %#p %#p\n",
            (*pv)->name, v->name, (*pv)->isr, v->isr, (*pv)->eoi, v->eoi);
      goto Unlockandfree;
    }
    if (*pv == vclock)
      pv = &vclock->next;
...
void intrdisable(int irq, void (*f)(Ureg *, void *), void *a, int tbdf,
                 char *name) {
  Vctl **pv, *v;
  int vno;
  ...
  do {
    for (pv = &vctl[vno]; (v = *pv) != nil; pv = &v->next) {
      if ((v->irq == irq || irq == -1) && v->tbdf == tbdf && v->f == f &&
          v->a == a && strcmp(v->name, name) == 0)
        break;
    }
    if (v != nil) {
      if (v->disable != nil) {
...
static long irqallocread(Chan *, void *a, long n, vlong offset) {
...
void irqinit(void) { addarchfile("irqalloc", 0444, irqallocread, nil); }
