...
/*@
    requires limine_hhdm_offset == 0 || limine_hhdm_offset >=
   0xffff800000000000UL; assigns limine_hhdm_offset, limine_kernel_phys_base;
    assigns MemMin;
    assigns *(char*)(CONFADDR+0...BOOTLINELEN-1);
    ensures MemMin > 0;
*/
void bootargsinit(void) {
  ...
  if (limine_memmap && limine_memmap->response) {
    ...
    /*@
        loop invariant 0 <= i <= memmap_response->entry_count;
        loop assigns i, entry, max_addr;
        loop variant memmap_response->entry_count - i;
    */
