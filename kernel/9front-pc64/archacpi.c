...
static ushort get16(uchar *p) { return p[1] << 8 | p[0]; }

static uint get32(uchar *p) {
  return p[3] << 24 | p[2] << 16 | p[1] << 8 | p[0];
}

static uvlong get64(uchar *p) {
  uvlong u;
...
static uint tbldlen(Tbl *t) { return get32(t->len) - Tblsz; }

static long memcheck(uintptr pa, long len) {
  int i;
  uintptr pe;
  Confmem *cm;
...
static void maptable(uvlong pa) {
  uchar *p, *e;
  u32int l;
  Tbl *t;
  int i;
...
static Tbl *findtable(char sig[4]) {
  Tbl *t;
  int i;
...
static Apic *findapic(int gsi, int *pintin) {
  Apic *a;
  int i;
...
static void addirq(int gsi, int type, int busno, int irq, int flags) {
  Apic *a;
  Bus *bus;
  Aintr *ai;
  PCMPintr *pi;
...
static int getirqs(void *d, uchar pmask[32], int *pflags) {
  int i, n, m;
  uchar *p;
...
static uchar *setirq(void *d, uint irq) {
  uchar *p;
...
static int setuplink(void *link, int *pflags) {
...
static int enumprt(void *dot, void *) {
  void *p, **a, **b;
...
static int enumec(void *dot, void *) {
...
static long readmem(Chan *, void *v, long n, vlong o) {
  uvlong pa = (uvlong)o;
  void *t;
...
static long writemem(Chan *, void *v, long n, vlong o) {
  uvlong pa = (uvlong)o;
  void *t;
...
/*@
    requires \true;
    // assigns ... global ACPI state
*/
static void acpiinit(void) {
  Tbl *t;
  Apic *a;
  void *va;
  uchar *s, *p, *e;
...
static long readtbls(Chan *, void *v, long n, vlong o) {
  int i, l, m;
  uchar *p;
  Tbl *t;
...
static int identify(void) {
  ...
  if (rsd == nil) {
    print("ACPI: no RSDP found, returning 1\n");
    return 1;
  }
  if (checksum(rsd, 20) && checksum(rsd, 36)) {
    print("ACPI: RSDP checksum failed, returning 1\n");
    return 1;
  }
  print("ACPI: RSDP found, mapping tables\n");
  maptables();
  print("ACPI: mapped %d tables\n", ntblmap);
  addarchfile("acpitbls", 0444, readtbls, nil);
  addarchfile("acpimem", 0600, readmem, writemem);
  t = findtable("APIC");
  if (v == 0 || t == nil) {
    print("ACPI: no APIC table found (t=%p), returning 1\n", t);
    return 1;
  }
  print("ACPI: APIC table found, using archacpi\n");
  if ((cp = getconf("*nomp")) != nil && strcmp(cp, "0") != 0)
    return 1;
  if (getconf("*nohpet") == nil && (t = findtable("HPET")) != nil &&
      ((uchar *)t)[40] == 0 && hpetprobe(get64((uchar *)t + 44)) == 0) {
...
static int readpcicfg(Amlio *io, void *data, int n, int offset) {
  ulong r, x;
  Pcidev *p;
  uchar *a;
  int i;
...
static int readec(Amlio *io, void *data, int n, int off) {
  int port, v;
  uchar *p;
...
static int writeec(Amlio *io, void *data, int n, int off) {
  int port;
  uchar *p;
...
static int writepcicfg(Amlio *io, void *data, int n, int offset) {
  ulong r, x;
  Pcidev *p;
  uchar *a;
  int i;
...
static int readioport(Amlio *io, void *data, int len, int port) {
  uchar *a;
...
static int writeioport(Amlio *io, void *data, int len, int port) {
  uchar *a;
