...
/*@
  @ assigns \nothing;
  @*/
static inline void early_outb(int port, uchar data) { outb(port, data); }

/*@
  @ assigns \nothing;
  @*/
static inline uchar early_inb(int port) { return inb(port); }
...
/*@
  @ requires valid_string_or_null(fmt);
  @ assigns \nothing; // simplified hardware access
  @*/
void early_iprint(char *fmt, ...) {
  ...
  /*@ loop invariant 0 <= i <= n;
    @ loop assigns i; // Simplified, outputs to hardware
    @ loop variant n - i;
    @*/
  for (i = 0; i < n; i++) {
    if (buf[i] == '\n')
