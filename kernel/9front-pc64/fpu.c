...
mathemu(Ureg *ureg, void*)
{
	if(!userureg(ureg)){
		if(up == nil){
			...
			case FPinit:
				...
				break;
			case FPinactive:
				fpcheck((FPsave*)m->fpsave, 1);
				fprestore((FPsave*)m->fpsave);
				m->fpstate = FPactive;
				break;
			default:
		...
		case FPinit:
			...
			break;
		case FPinactive:
			fpcheck((FPsave*)up->kfpsave, 1);
			fprestore((FPsave*)up->kfpsave);
			up->kfpstate = FPactive;
			break;
		default:
	...
	case FPinactive|FPnotify:
		...
		/* wet floor */
	case FPinactive:
		if(fpcheck((FPsave*)up->fpsave, 0))
			break;
		fprestore((FPsave*)up->fpsave);
		up->fpstate = FPactive;
		break;
	case FPprotected:
		up->fpstate = FPactive;
