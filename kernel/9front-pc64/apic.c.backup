 ...
 */
static void lapictimerinit(void) {
  uvlong x, v, hz;
  Apictimer *a;
  int s;
...
Retry:
   ...
   * or calibrate to a very low value, causing an interrupt storm.
   * Enforce a reasonable minimum (10MHz).
   */
  if (v < 10000000ULL) {
    print(
        "lapictimerinit: calibration anomaly (v=%llud), defaulting to 100MHz\n",
        v);
    v = 100000000ULL;
  }
...
void lapicstartap(Apic *apic, int v) {
  int i;
  ulong crhi;
...
void lapicicrw(ulong hi, ulong lo) {
  lapicw(LapicICRHI, hi);
  lapicw(LapicICRLO, lo);
}

void ioapicrdtr(Apic *apic, int sel, int *hi, int *lo) {
  ulong *iowin;
...
void ioapicrdtw(Apic *apic, int sel, int hi, int lo) {
  ulong *iowin;
...
void lapictimerset(uvlong next) {
  vlong period;
  Apictimer *a;
...
void lapicintron(void) { lapicw(LapicTPR, 0); }

void lapicintroff(void) { lapicw(LapicTPR, 0xFF); }
