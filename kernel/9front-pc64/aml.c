...
static char *spacename[] = {
    "Mem", "Io", "Pcicfg", "Ebctl", "Smbus", "Cmos", "Pcibar", "Ipmi",
};
...
static Name *rootname(Name *dot) {
  while (dot != dot->up)
    dot = dot->up;
  return dot;
}

static void gcmark(void *p) {
  int i;
  Env *e;
  Field *f;
  Heap *h;
      ...
      if (r->mapped > 0) {
        if (amldebug)
          print("\namlunmapio(%N): %-8s %llux - %llux\n", (Name *)r->name,
                spacename[r->space], r->off, r->off + r->len);
        amlunmapio(r);
      }
      r->mapped = 0;
      r->write = nil;
...
static void *mk(int tag, int size) {
  Heap *h;
  int a;
...
static char *mks(char *s) {
  char *r = mk('s', strlen(s) + 1);
  strcpy(r, s);
  return r;
}

static int pkglen(uchar *p, uchar *e, uchar **np) {
  ulong n;
  uchar b;
...
static Name *getseg(Name *dot, void *seg, int new) {
  Name *n, *l;
...
Name *getname(Name *dot, char *path, int new) {
  char seg[4];
  int i, s;
  Name *x;
...
static int fixnames(void *dot, void *arg) {
  void **r, *v;
  int i;
...
static uvlong getle(uchar *p, int len) {
  uvlong v;
  int i;
...
static void putle(uchar *p, int len, uvlong v) {
  int i;
...
static uvlong rwreg(void *reg, int off, int len, uvlong v, int write) {
  Interp save;
  uchar buf[8], *p;
  Region *r;
    ...
    if (write) {
      if (amldebug)
        print("\nrwreg(%N): %-8s [%llux+%x]/%d <- %llux\n", (Name *)r->name,
              spacename[r->space], r->off, off, len, v);
      putle(p, len, v);
      if (r->write != nil) {
...
static int fieldalign(int flags) {
...
static uvlong rwfieldunit(Field *f, int off, int len, uvlong v, int write) {
  if (f->index) {
    if (TAG(f->reg) == 'f') {
      /* set index field */
...
static char *todecstr(uchar *buf, int len, int sep) {
  char *r, *d;
  int i, v;
...
static char *tohexstr(uchar *buf, int len, int sep) {
  char *r, *d;
  int i;
...
static void *copy(int tag, void *s) {
  uvlong v;
  void *d;
  int n;
...
static int Vfmt(Fmt *f) {
  ...
  case 'r':
    g = p;
    return fmtprint(f, "Region(%s, %#llux, %#llux)", spacename[g->space & 7],
                    g->off, g->len);
  case 'm':
    m = p;
    fmtprint(f, "%N(", m->name);
  ...
  case 'f':
    ...
    if (l->index) {
      if (TAG(l->reg) == 'f')
        return fmtprint(f, "IndexField(%x, %x, %x) @ %V[%V]", l->flags,
                        l->bitoff, l->bitlen, l->reg, l->index);
      else
        return fmtprint(f, "BankField(%x, %x, %x, %V=%V) @ %V", l->flags,
                        l->bitoff, l->bitlen, l->index, l->bank, l->reg);
    }
    return fmtprint(f, "Field(%x, %x, %x) @ %V", l->flags, l->bitoff, l->bitlen,
                    l->reg);
  default:
...
static int xec(uchar *pc, uchar *end, Name *dot, Env *env, void **pret) {
  static int loop;
  int i, c;
  void *r;
...
static void *evaliarg0(void) { return FP->arg[0]; }

static void *evalconst(void) {
  switch (FP->start[0]) {
  case 0x01:
    return mki(1);
  case 0xFF:
...
static void *evalnop(void) { return nil; }

static void *evalbad(void) {
  int i;
...
static vlong cmp1(void *a, void *b) {
  vlong c;
  int tag;
...
static void *evalstore(void) { return store(FP->arg[0], FP->arg[1]); }

static void *evalcat(void) {
  void *r, *a, *b;
  int tag, n, m;
...
static int match1(int op, void *a, void *b) {
  vlong c = cmp1(a, b);
  switch (op) {
  case 0:
    return 1;
...
static void *evalsize(void) { return mki(amllen(FP->arg[0])); }

static void *evalderef(void) {
  void *p;
...
static void *evalstall(void) {
  amldelay(ival(FP->arg[0]));
  return nil;
}

static void *evalsleep(void) {
  amldelay(ival(FP->arg[0]) * 1000);
  return nil;
}

static void *evalconv(void) {
  void *r, *a;
...
static Op optab[] = {
    ...
    "*iiN",
    evalcfield,
    [Ocfld0] "CreateBitField",
    "*iN",
    evalcfield,
    [Ocfld1] "CreateByteField",
    "*iN",
    evalcfield,
    [Ocfld2] "CreateWordField",
    "*iN",
    evalcfield,
    [Ocfld4] "CreateDWordField",
    "*iN",
    evalcfield,
    [Ocfld8] "CreateQWordField",
    "*iN",
    evalcfield,
    ...
    [Olbit] "FindSetLeftBit",
    "i@",
    evalarith,
    [Orbit] "FindSetRightBit",
    "i@",
    evalarith,
...
static uchar octab2[] = {
    /* 00 */ Obad,  Omutex, Oevent,  Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 08 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 10 */ Obad,  Obad,   Ocref,   Ocfld,
    Obad,           Obad,   Obad,    Obad,
    /* 18 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 20 */ Oload, Ostall, Osleep,  Oacq,
    Osignal,        Obad,   Obad,    Orel,
    /* 28 */ Obad,  Obad,   Ounload, Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 30 */ Obad,  Odebug, Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 38 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 40 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 48 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 50 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 58 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 60 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 68 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 70 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 78 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 80 */ Oreg,  Ofld,   Odev,    Ocpu,
    Oprc,           Othz,   Oxfld,   Obfld,
    /* 88 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 90 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* 98 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* A0 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* A8 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* B0 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* B8 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* C0 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* C8 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* D0 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* D8 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* E0 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* E8 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* F0 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
    /* F8 */ Obad,  Obad,   Obad,    Obad,
    Obad,           Obad,   Obad,    Obad,
};

int amltag(void *p) { return p ? TAG(p) : 0; }

void *amlval(void *p) { return deref(p); }

uvlong amlint(void *p) { return ival(p); }
...
/*@
    requires \true;
    assigns amlroot, hp; // and potentially others
    ensures amlroot != \null;
*/
void amlinit(void) {
  Name *n;
...
/*@
    requires data != \null;
    requires len >= 0;
    // Security: Input Validation - Ensure we only read within the provided
   buffer requires \valid_read(data + (0..len-1)); assigns hp; // Abstract
   interpretation state ensures \true;
*/
...
void *amlwalk(void *dot, char *name) { return getname(dot, name, 0); }
