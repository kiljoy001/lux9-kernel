 ...
 * They are used for formal verification of the borrow checker's safety properties.
 */
/*@
    ...
    \exists ulong h; h == borrow_hash(o->key) && h == hash &&
      \exists struct BorrowOwner *curr; curr == borrowpool.owners[hash].head &&
        (\forall struct BorrowOwner *tmp; tmp == curr ==> 
          (tmp == o || \exists struct BorrowOwner *next; tmp->next == next && in_chain(next, o)));
  predicate in_chain(struct BorrowOwner *curr, struct BorrowOwner *target) =
    curr == target || 
    (curr != \null && curr->next != \null && in_chain(curr->next, target));
  predicate in_shared_list(struct SharedBorrower *sb, Proc *p) =
    sb != \null && (sb->proc == p || in_shared_list(sb->next, p));
*/
/* Global borrow pool invariants matching Coq's ValidState */
/*@
 ...
 */
static Lock dag_locks[64];
/*
 * Helpers
 */
static void lock_dag(MsgOrd *dag) {
  if (dag && dag->gd_id >= 0 && dag->gd_id < 64)
...
MsgOrd *msgord_create_instance(uint k_param) {
  ...
  dag->gd_id = id;
  dag->gd_k_param = k_param ? k_param : 3;
  dag->gd_max_anticone = 10;
  dag->gd_next_id = 1;
  dag->gd_global_seq = 0;
...
static OrdMsg *msgord_alloc(MsgOrd *dag, Proc *caller, char *path) {
  ...
  msg->gm_state = 0;
  msg->gm_color = 1; /* Default optimistic */
  if (path != ((void *)0))
 ...
 */
uint msgord_add_message(msgord_state_t *state, Proc *p, Fcall *t, char *path) {
  if (state) { }; /* In original code, but now we respect state if passed */
  if (state == ((void *)0))
    state = msgord;
...
OrdMsg *msgord_next(MsgOrd *dag) {
  ...
  for (msg = dag->gd_head; msg != ((void *)0); msg = msg->gm_next) {
    if (msg->gm_state == 1 && msgord_can_deliver(dag, msg)) {
      unlock_dag(dag);
      return msg;
    }
  }
...
void msgord_complete(MsgOrd *dag, OrdMsg *msg) {
  ...
  if ((msg->gm_payload.type == 1 ||
       msg->gm_payload.type == 0) &&
...
int msgord_fire_completions(MsgOrd *dag) {
  ...
  for (msg = dag->gd_head; msg != ((void *)0); msg = next) {
    ...
    if (!msgord_can_deliver(dag, msg))
      continue;
    /* Process the message */
    if (msg->gm_payload.type == 0 && msg->gm_caller) {
    ...
    if (msg->gm_callback != ((void *)0)) {
      unlock_dag(dag);
      msg->gm_callback(msg, 0, msg->gm_callback_arg);
      lock_dag(dag);
      fired++;
    }
    /* Remove from DAG */
...
int msgord_check_consensus_depth(MsgOrd *dag, uint op_id, int required_depth,
  ...
  if (required_depth) { }; /* May adjust confidence threshold in future */
...
int msgord_submit_async_depth(MsgOrd *dag, Proc *caller, void *t, void *r,
  ...
  uint msg_id;
  if (r) { }; /* Reply will be filled by MSGORD processing */
  if (depth) { }; /* Depth is used for rollback registration in consensus_depth.c */
  if (dag == ((void *)0))
    dag = msgord;
  if (dag == ((void *)0) || fcall_t == ((void *)0))
    return -1;
  /* Submit via existing async mechanism */
  msg_id = msgord_submit_async(dag, caller, fcall_t, path, ((void *)0), ((void *)0), nonce);
  if (msg_id == 0)
    return -1;
