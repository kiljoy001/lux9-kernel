...
void wasi_lux9_destroy_context(wasi_context_t *ctx) {
  ...
  for (int i = 0; i < WASI_MAX_FDS; i++) {
    if (ctx->fds[i].is_open) {
      ...
      if (ctx->fds[i].base_path &&
          ctx->fds[i].base_path != wasi_root_path &&
...
m3ApiRawFunction(wasi_snapshot_preview1_fd_prestat_get) {
  ...
  m3ApiWriteMem32(prestat_ptr + 4,
                  (fd == 4) ? (sizeof(wasi_posix_root_path) - 1)
...
m3ApiRawFunction(wasi_snapshot_preview1_path_open) {
  ...
  */
  ...
  char *fullpath = nil;
  uint32_t perr =
      wasi_build_path(ctx, dirfd, path, path_len, &fullpath);
...
m3ApiRawFunction(wasi_snapshot_preview1_fd_close) {
  ...
  if (ctx->fds[fd].base_path &&
      ctx->fds[fd].base_path != wasi_root_path &&
...
m3ApiRawFunction(wasi_snapshot_preview1_fd_renumber) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, from)
      m3ApiGetArg(int32_t, to)
  ...
  if (ctx->fds[to].is_open) {
    ...
    if (ctx->fds[to].base_path &&
        ctx->fds[to].base_path != wasi_root_path &&
...
m3ApiRawFunction(wasi_snapshot_preview1_fd_fdstat_get) {
  ...
  uint8_t filetype = (fd <= 2) ? WASI_FILETYPE_CHARACTER_DEVICE
...
m3ApiRawFunction(wasi_snapshot_preview1_fd_tell) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, fd)
      m3ApiGetArg(uint32_t, offset_ptr)
          Proc *p = up;
...
m3ApiRawFunction(wasi_snapshot_preview1_fd_filestat_set_times) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, fd)
      m3ApiGetArg(uint64_t, atim) m3ApiGetArg(uint64_t, mtim)
          m3ApiGetArg(uint32_t, flags)
              Proc *p = up;
  (void)atim;
  (void)mtim;
...
/* wasi_path_filestat_set_times(dirfd, dirflags, path, path_len, atim, mtim, flags) */
m3ApiRawFunction(wasi_snapshot_preview1_path_filestat_set_times) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, dirfd)
      m3ApiGetArg(uint32_t, dirflags) m3ApiGetArg(uint32_t, path_ptr)
          m3ApiGetArg(uint32_t, path_len) m3ApiGetArg(uint64_t, atim)
              m3ApiGetArg(uint64_t, mtim) m3ApiGetArg(uint32_t, flags)
                  Proc *p = up;
  (void)dirflags;
  (void)path_ptr;
  (void)path_len;
  ...
  int r = wasi_rump_path_req("/srv/rump/posix/path_set_times",
                             POSIX_PATH_SET_TIMES, posix_path, full_len,
                             extra, sizeof(extra),
                             &resp, sizeof(resp));
  free(fullpath);
  if (r < (int)sizeof(resp))
...
m3ApiRawFunction(wasi_snapshot_preview1_path_filestat_set_size) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, dirfd)
      m3ApiGetArg(uint32_t, dirflags) m3ApiGetArg(uint32_t, path_ptr)
          m3ApiGetArg(uint32_t, path_len) m3ApiGetArg(uint64_t, size)
              Proc *p = up;
  (void)dirflags;
  (void)path_ptr;
  (void)path_len;
  ...
  wasi_context_t *ctx = (wasi_context_t *)p->wasm.wasi_ctx;
  uint32_t cap =
      wasi_require_fd(ctx, dirfd, WASI_RIGHT_PATH_FILESTAT_SET_SIZE);
  if (cap != WASI_ERRNO_SUCCESS)
    m3ApiReturn(cap);
  if (!ctx->fds[dirfd].is_dir)
    m3ApiReturn(WASI_ERRNO_NOTDIR);
  ...
  int r = wasi_rump_path_req("/srv/rump/posix/path_set_size",
                             POSIX_PATH_SET_SIZE, posix_path, full_len,
                             extra, sizeof(extra),
                             &resp, sizeof(resp));
  free(fullpath);
  if (r < (int)sizeof(resp))
...
m3ApiRawFunction(wasi_snapshot_preview1_path_create_directory) {
  ...
  wasi_context_t *ctx = (wasi_context_t *)p->wasm.wasi_ctx;
  uint32_t cap =
      wasi_require_fd(ctx, dirfd, WASI_RIGHT_PATH_CREATE_DIRECTORY);
  if (cap != WASI_ERRNO_SUCCESS)
    m3ApiReturn(cap);
  if (!ctx->fds[dirfd].is_dir)
    m3ApiReturn(WASI_ERRNO_NOTDIR);
...
m3ApiRawFunction(wasi_snapshot_preview1_path_remove_directory) {
  ...
  wasi_context_t *ctx = (wasi_context_t *)p->wasm.wasi_ctx;
  uint32_t cap =
      wasi_require_fd(ctx, dirfd, WASI_RIGHT_PATH_REMOVE_DIRECTORY);
  if (cap != WASI_ERRNO_SUCCESS)
    m3ApiReturn(cap);
  if (!ctx->fds[dirfd].is_dir)
    m3ApiReturn(WASI_ERRNO_NOTDIR);
  ...
  uint32_t resp = 0;
  int r = wasi_rump_path_req("/srv/rump/posix/path_remove_directory",
                             POSIX_PATH_REMOVE_DIRECTORY, posix_path, full_len,
                             nil, 0, &resp,
                             sizeof(resp));
  free(fullpath);
  if (r < (int)sizeof(resp))
...
m3ApiRawFunction(wasi_snapshot_preview1_path_unlink_file) {
  ...
  wasi_context_t *ctx = (wasi_context_t *)p->wasm.wasi_ctx;
  uint32_t cap =
      wasi_require_fd(ctx, dirfd, WASI_RIGHT_PATH_UNLINK_FILE);
  if (cap != WASI_ERRNO_SUCCESS)
    m3ApiReturn(cap);
  if (!ctx->fds[dirfd].is_dir)
    m3ApiReturn(WASI_ERRNO_NOTDIR);
  ...
  uint32_t resp = 0;
  int r = wasi_rump_path_req("/srv/rump/posix/path_unlink_file",
                             POSIX_PATH_UNLINK_FILE, posix_path, full_len, nil,
                             0, &resp,
                             sizeof(resp));
  free(fullpath);
  if (r < (int)sizeof(resp))
...
m3ApiRawFunction(wasi_snapshot_preview1_path_rename) {
  ...
  wasi_context_t *ctx = (wasi_context_t *)p->wasm.wasi_ctx;
  uint32_t cap =
      wasi_require_fd(ctx, old_fd, WASI_RIGHT_PATH_RENAME_SOURCE);
  if (cap != WASI_ERRNO_SUCCESS)
    m3ApiReturn(cap);
  ...
  char *new_full = nil;
  uint32_t perr =
      wasi_build_path(ctx, old_fd, oldp, old_len, &old_full);
  if (perr != WASI_ERRNO_SUCCESS)
    m3ApiReturn(perr);
...
m3ApiRawFunction(wasi_snapshot_preview1_path_link) {
  ...
  wasi_context_t *ctx = (wasi_context_t *)p->wasm.wasi_ctx;
  uint32_t cap =
      wasi_require_fd(ctx, old_fd, WASI_RIGHT_PATH_LINK_SOURCE);
  if (cap != WASI_ERRNO_SUCCESS)
    m3ApiReturn(cap);
...
m3ApiRawFunction(wasi_snapshot_preview1_path_symlink) {
  ...
  char *new_full = nil;
  uint32_t perr =
      wasi_build_path(ctx, dirfd, newp, new_len, &new_full);
  if (perr != WASI_ERRNO_SUCCESS)
    m3ApiReturn(perr);
...
m3ApiRawFunction(wasi_snapshot_preview1_path_readlink) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, dirfd)
      m3ApiGetArg(uint32_t, path_ptr) m3ApiGetArg(uint32_t, path_len)
          m3ApiGetArg(uint32_t, buf_ptr) m3ApiGetArg(uint32_t, buf_len)
              m3ApiGetArg(uint32_t, bufused_ptr)
                  Proc *p = up;
  (void)path_ptr;
  (void)path_len;
  (void)buf_ptr;
  ...
  int r = wasi_rump_path_req("/srv/rump/posix/path_readlink",
                             POSIX_PATH_READLINK, posix_path, full_len,
                             &extra,
...
m3ApiRawFunction(wasi_snapshot_preview1_sock_accept) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, fd)
      m3ApiGetArg(uint32_t, flags) m3ApiGetArg(uint32_t, newfd_ptr)
          Proc *p = up;
  (void)flags;
...
m3ApiRawFunction(wasi_snapshot_preview1_sock_recv) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, fd)
      m3ApiGetArg(uint32_t, ri_data) m3ApiGetArg(uint32_t, ri_data_len)
          m3ApiGetArg(uint32_t, ri_flags)
              m3ApiGetArg(uint32_t, ro_datalen)
                  m3ApiGetArg(uint32_t, ro_flags)
                      Proc *p = up;
  ...
  int r = wasi_rump_rpc_read("/srv/rump/posix/sock_recv", req, sizeof(req), resp,
                             12 + total);
  if (r < 4)
    m3ApiReturn(WASI_ERRNO_IO);
...
m3ApiRawFunction(wasi_snapshot_preview1_sock_send) {
  m3ApiReturnType(uint32_t) m3ApiGetArg(int32_t, fd)
      m3ApiGetArg(uint32_t, si_data) m3ApiGetArg(uint32_t, si_data_len)
          m3ApiGetArg(uint32_t, si_flags)
              m3ApiGetArg(uint32_t, so_datalen)
                  Proc *p = up;
...
m3ApiRawFunction(wasi_snapshot_preview1_fd_filestat_get) {
  ...
  if (wasi_fd_backend(ctx, fd) == WASI_BACKEND_POSIX) {
    ...
    uint8_t filetype =
        (d->mode & DMDIR) ? WASI_FILETYPE_DIRECTORY : WASI_FILETYPE_REGULAR_FILE;
  ...
  int r =
      wasi_rump_rpc("/srv/rump/posix/fdstat", req, sizeof(req), resp,
...
M3Result LinkWasi(IM3Module module) {
  ...
  result = m3_LinkRawFunction(module, wasi, "args_sizes_get", "i(**)",
                              &wasi_snapshot_preview1_args_sizes_get);
  if (result)
    return result;

  // args_get: (i32*, i32) -> i32
  result = m3_LinkRawFunction(module, wasi, "args_get", "i(**)",
                              &wasi_snapshot_preview1_args_get);
  if (result)
    return result;

  // environ_sizes_get: (i32*, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "environ_sizes_get", "i(**)",
                              &wasi_snapshot_preview1_environ_sizes_get);
  if (result)
    return result;

  // environ_get: (i32*, i32) -> i32
  result = m3_LinkRawFunction(module, wasi, "environ_get", "i(**)",
                              &wasi_snapshot_preview1_environ_get);
  if (result)
    return result;

  // fd_prestat_get: (i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_prestat_get", "i(i*)",
                              &wasi_snapshot_preview1_fd_prestat_get);
  if (result)
    return result;

  // fd_prestat_dir_name: (i32, i32, i32) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_prestat_dir_name", "i(i*i)",
                              &wasi_snapshot_preview1_fd_prestat_dir_name);
  if (result)
    return result;

  // random_get: (i32, i32) -> i32
  result = m3_LinkRawFunction(module, wasi, "random_get", "i(*i)",
                              &wasi_snapshot_preview1_random_get);
  if (result)
    return result;

  // clock_res_get: (i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "clock_res_get", "i(i*)",
                              &wasi_snapshot_preview1_clock_res_get);
  if (result)
    return result;

  // clock_time_get: (i32, i64, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "clock_time_get", "i(iI*)",
                              &wasi_snapshot_preview1_clock_time_get);
  if (result)
    return result;

  // fd_write: (i32, i32*, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_write", "i(i*i*)",
                              &wasi_snapshot_preview1_fd_write);
  if (result)
    return result;

  // fd_read: (i32, i32*, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_read", "i(i*i*)",
                              &wasi_snapshot_preview1_fd_read);
  if (result)
    return result;

  // fd_pread: (i32, i32*, i32, i64, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_pread", "i(i*iI*)",
                              &wasi_snapshot_preview1_fd_pread);
  if (result)
    return result;

  // fd_pwrite: (i32, i32*, i32, i64, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_pwrite", "i(i*iI*)",
                              &wasi_snapshot_preview1_fd_pwrite);
  if (result)
    return result;

  result = m3_LinkRawFunction(module, wasi, "fd_advise", "i(iIIi)",
                              &wasi_snapshot_preview1_fd_advise);
  if (result)
    return result;

  result = m3_LinkRawFunction(module, wasi, "fd_allocate", "i(iII)",
                              &wasi_snapshot_preview1_fd_allocate);
  if (result)
    return result;

  // fd_seek: (i32, i64, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_seek", "i(iIi*)",
  ...
  result = m3_LinkRawFunction(module, wasi, "fd_renumber", "i(ii)",
  ...
  result = m3_LinkRawFunction(module, wasi, "proc_raise", "i(i)",
                              &wasi_snapshot_preview1_proc_raise);
  if (result)
    return result;

  result = m3_LinkRawFunction(module, wasi, "sched_yield", "i()",
                              &wasi_snapshot_preview1_sched_yield);
  if (result)
    return result;

  // path_open: (i32, i32, i32*, i32, i32, i64, i64, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "path_open", "i(ii*iiIIi*)",
                              &wasi_snapshot_preview1_path_open);
  if (result)
    return result;

  // fd_fdstat_get: (i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_fdstat_get", "i(i*)",
                              &wasi_snapshot_preview1_fd_fdstat_get);
  if (result)
    return result;

  result = m3_LinkRawFunction(module, wasi, "fd_fdstat_set_flags", "i(ii)",
                              &wasi_snapshot_preview1_fd_fdstat_set_flags);
  if (result)
    return result;

  result = m3_LinkRawFunction(module, wasi, "fd_fdstat_set_rights", "i(iII)",
                              &wasi_snapshot_preview1_fd_fdstat_set_rights);
  if (result)
    return result;

  // fd_readdir: (i32, i32*, i32, i64, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_readdir", "i(i*iI*)",
                              &wasi_snapshot_preview1_fd_readdir);
  if (result)
    return result;

  // fd_sync: (i32) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_sync", "i(i)",
                              &wasi_snapshot_preview1_fd_sync);
  if (result)
    return result;

  result = m3_LinkRawFunction(module, wasi, "fd_datasync", "i(i)",
                              &wasi_snapshot_preview1_fd_datasync);
  if (result)
    return result;

  // fd_tell: (i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_tell", "i(i*)",
                              &wasi_snapshot_preview1_fd_tell);
  if (result)
    return result;

  // fd_filestat_get: (i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_filestat_get", "i(i*)",
                              &wasi_snapshot_preview1_fd_filestat_get);
  if (result)
    return result;

  // fd_filestat_set_size: (i32, i64) -> i32
  result = m3_LinkRawFunction(module, wasi, "fd_filestat_set_size", "i(iI)",
  ...
  result = m3_LinkRawFunction(module, wasi, "path_filestat_set_size",
                              "i(ii*iI)",
  ...
  result = m3_LinkRawFunction(module, wasi, "path_rename", "i(i*ii*i)",
                              &wasi_snapshot_preview1_path_rename);
  if (result)
    return result;

  result = m3_LinkRawFunction(module, wasi, "path_link", "i(i*ii*i)",
                              &wasi_snapshot_preview1_path_link);
  if (result)
    return result;

  // path_symlink: (i32*, i32, i32, i32*, i32) -> i32
  result = m3_LinkRawFunction(module, wasi, "path_symlink", "i(*ii*i)",
                              &wasi_snapshot_preview1_path_symlink);
  if (result)
    return result;

  // path_readlink: (i32, i32*, i32, i32*, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "path_readlink", "i(i*i*i*)",
                              &wasi_snapshot_preview1_path_readlink);
  if (result)
    return result;

  // poll_oneoff: (i32*, i32*, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "poll_oneoff", "i(**i*)",
                              &wasi_snapshot_preview1_poll_oneoff);
  if (result)
    return result;

  // sock_accept: (i32, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "sock_accept", "i(ii*)",
                              &wasi_snapshot_preview1_sock_accept);
  if (result)
    return result;

  // sock_recv: (i32, i32*, i32, i32, i32*, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "sock_recv", "i(i*ii**)",
                              &wasi_snapshot_preview1_sock_recv);
  if (result)
    return result;

  // sock_send: (i32, i32*, i32, i32, i32*) -> i32
  result = m3_LinkRawFunction(module, wasi, "sock_send", "i(i*ii*)",
                              &wasi_snapshot_preview1_sock_send);
  if (result)
    return result;

  // sock_shutdown: (i32, i32) -> i32
  result = m3_LinkRawFunction(module, wasi, "sock_shutdown", "i(ii)",
