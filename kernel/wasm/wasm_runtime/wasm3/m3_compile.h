...
typedef struct {
  ...
  u16 stackFirstDynamicIndex; // args and locals are pushed to the stack so that
                              // their slot locations can be tracked. the wasm
                              // model itself doesn't treat these values as
                              // being on the stack, so stackFirstDynamicIndex
                              // marks the start of the real Wasm stack
...
static inline u8 GetSingleRetType(IM3FuncType ftype) {
  if (ftype && ftype->numRets > 0)
    return ftype->types[0];
  else
    return (u8)c_m3Type_none;
}
