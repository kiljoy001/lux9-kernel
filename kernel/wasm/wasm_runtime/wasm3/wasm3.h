...
/* Map standard C types to Plan 9 kernel types */
...
#if defined(__cplusplus)
#define d_m3ErrorConst(LABEL, STRING)                                          \
  extern const M3Result m3Err_##LABEL = {STRING};
#else
...
d_m3ErrorConst(none, NULL)
    ...
    d_m3ErrorConst(incompatibleWasmVersion, "incompatible Wasm binary version")
        d_m3ErrorConst(wasmMalformed, "malformed Wasm binary") d_m3ErrorConst(
            misorderedWasmSection, "out of order Wasm section")
            d_m3ErrorConst(wasmUnderrun, "underrun while parsing Wasm binary")
                d_m3ErrorConst(wasmOverrun, "overrun while parsing Wasm binary")
                    d_m3ErrorConst(wasmMissingInitExpr,
                        ...
                        d_m3ErrorConst(lebOverflow, "LEB encoded value "
                                                    "overflow") d_m3ErrorConst(
                            missingUTF8, "invalid length UTF-8 string")
                            d_m3ErrorConst(
                                wasmSectionUnderrun,
                                ...
                                d_m3ErrorConst(
                                    ...
                                    d_m3ErrorConst(invalidTypeId,
                                        ...
                                        d_m3ErrorConst(tooManyMemorySections,
                                                       "only one memory per "
                                                       "module is supported")
                                            d_m3ErrorConst(tooManyArgsRets,
                                                           "too many arguments "
                                                           "or return values")
