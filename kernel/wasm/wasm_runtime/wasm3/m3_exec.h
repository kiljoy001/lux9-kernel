...
d_m3Op(Const_v128) {
#if d_m3HasSIMD
  v128 val = immediate(v128); // Requires immediate() support for struct/vector
                              // types which might need casting
  // If immediate(TYPE) is just *(TYPE*)_pc++, we need to ensure alignment or
  // use memcpy Unaligned access is common in M3 immediate reading Let's assume
  // generic read for now: val = * ((v128*) _pc); _pc = (pc_t) ((v128*) _pc +
  // 1); But better to be explicit if macros assume scalars.

  // Using the macro expansion of immediate(v128):
  // #define immediate(TYPE) (* (TYPE *) (_pc++)) ? NO, _pc is pointer to code_t
  // (void*) M3 uses specific slot widths. Code stream is usually aligned?
  // Bytecode immediate is unaligned. Compiled code?
  // Compiled code 'immediate' operands are emitted into the stream.
  // They are word-aligned?
  // m3_compile uses EmitWord64 etc.

  // Let's look at Const_f64 implementation if possible or just assume immediate
  // work for standard types Since v128 is a custom type, standard cast might
  // work if defined in C. However, creating a safe implementation:
  ...
  push(v128, v);
#else
  return m3Err_wasmMissingInitExpr; // Should have trap for missing SIMD
#endif
  nextOp();
}
...
d_m3Op(MemCopy) {
  ...
  if (M3_LIKELY(destination + size <= _mem->length)) {
    if (M3_LIKELY(source + size <= _mem->length)) {
      /* Lux9: Charge Pebble Arena for bandwidth */
      if (M3_UNLIKELY(arena_branch_alloc(&up->wasm.branch, size) != 0))
        newTrap(m3Err_trapAbort); // Resource exhaustion
...
d_m3Op(MemInit) {
  u32 dataSegmentIdx = immediate(u32);
  u32 size = (u32)_r0;
  u32 source = slot(u32);
  ...
  if (M3_UNLIKELY(dataSegmentIdx >= module->numDataSegments))
    newTrap(m3Err_trapOutOfBoundsMemoryAccess);

  M3DataSegment *segment = &module->dataSegments[dataSegmentIdx];

  if (M3_UNLIKELY(segment->dropped))
    newTrap(m3Err_trapOutOfBoundsMemoryAccess);

  if (M3_UNLIKELY(source + size > segment->size))
    newTrap(m3Err_trapOutOfBoundsMemoryAccess);

  if (M3_UNLIKELY(destination + size > _mem->length))
    newTrap(m3Err_trapOutOfBoundsMemoryAccess);

  /* Lux9: Charge Pebble Arena for bandwidth */
  if (M3_UNLIKELY(arena_branch_alloc(&up->wasm.branch, size) != 0))
    newTrap(m3Err_trapAbort);

  u8 *dst = m3MemData(_mem) + destination;
  const u8 *src = segment->data + source;
  memmove(dst, src, size);
...
d_m3Op(DataDrop) {
  ...
  if (M3_LIKELY(dataSegmentIdx < module->numDataSegments)) {
    module->dataSegments[dataSegmentIdx].dropped = true;
  }
  nextOp();
}

d_m3Op(TableInit) {
  u32 elementSegmentIdx = immediate(u32);
  u32 tableIdx = immediate(u32);
  u32 size = (u32)_r0;
  u32 source = slot(u32);
  u32 destination = slot(u32);
  ...
  if (M3_UNLIKELY(elementSegmentIdx >= module->numElementSegments))
    newTrap(m3Err_trapTableIndexOutOfRange);

  M3ElementSegment *segment = &module->elementSegments[elementSegmentIdx];

  if (M3_UNLIKELY(segment->dropped))
    newTrap(m3Err_trapTableIndexOutOfRange);

  if (M3_UNLIKELY(source + size > segment->numElements))
    newTrap(m3Err_trapTableIndexOutOfRange);

  if (M3_UNLIKELY(tableIdx != 0 ||
                  (u64)destination + size > module->table0Size))
    newTrap(m3Err_trapTableIndexOutOfRange);

  /* Lux9: Charge Pebble Arena for table operations */
  if (M3_UNLIKELY(arena_branch_alloc(&up->wasm.branch, size * sizeof(void *)) !=
                  0))
    newTrap(m3Err_trapAbort);
  ...
  if (size) {
    ...
    bytes_t ptr = segment->elements;
    // Skip elements
    for (u32 i = 0; i < source; ++i) {
      u32 skipIdx;
      M3Result res = ReadLEB_u32(&skipIdx, &ptr, module->wasmEnd);
      if (M3_UNLIKELY(res))
        return res;
    }
    ...
    for (u32 i = 0; i < size; ++i) {
      u32 funcIdx;
      M3Result res = ReadLEB_u32(&funcIdx, &ptr, module->wasmEnd);
      if (M3_UNLIKELY(res))
        return res;
      module->table0[destination + i] = &module->functions[funcIdx];
    }
  }
  nextOp();
}
...
d_m3Op(ElemDrop) {
  ...
  if (M3_LIKELY(elementSegmentIdx < module->numElementSegments)) {
    module->elementSegments[elementSegmentIdx].dropped = true;
  }
  nextOp();
}

d_m3Op(TableCopy) {
  u32 targetTableIdx = immediate(u32);
  u32 sourceTableIdx = immediate(u32);
  u32 size = (u32)_r0;
  u32 source = slot(u32);
  u32 destination = slot(u32);
  ...
  if (M3_UNLIKELY(targetTableIdx != 0 || sourceTableIdx != 0))
    newTrap(m3Err_trapTableIndexOutOfRange);

  if (M3_UNLIKELY((u64)source + size > module->table0Size ||
                  (u64)destination + size > module->table0Size))
    newTrap(m3Err_trapTableIndexOutOfRange);

  /* Lux9: Charge Pebble Arena */
  if (M3_UNLIKELY(arena_branch_alloc(&up->wasm.branch, size * sizeof(void *)) !=
                  0))
    newTrap(m3Err_trapAbort);

  if (size) {
    memmove(module->table0 + destination, module->table0 + source,
            size * sizeof(void *));
  }
  nextOp();
}
...
#endif
  {
    ...
    if (r) {
      ...
      if (function->funcType->numRets == 0) {
        ...
        int nRets = function->funcType->numRets;
        u64 *stack = (u64 *)_sp;

        for (int i = 0; i < nRets; ++i) {
          u8 type = function->funcType->types[i];
          p += SPrintArg(p, e - p, stack + i, type);
          if (i < nRets - 1)
            p += snprintf(p, e - p, ", ");
        }
