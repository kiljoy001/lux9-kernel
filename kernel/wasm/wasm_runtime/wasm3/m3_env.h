...
typedef struct M3DataSegment {
  ...
  u32 memoryRegion;
  u32 size;
  bool dropped; // Lux9: support data.drop
} M3DataSegment;
...
typedef struct M3ElementSegment {
  ...
  u32 tableIndex;
  u32 numElements;
  const u8 *elements; // points to the start of the element indices/exprs
  bool dropped;       // Lux9: support elem.drop
} M3ElementSegment;
...
typedef struct M3Environment {
  ...
  IM3FuncType funcTypes; // linked list of unique M3FuncType structs that can be
                         // compared using pointer-equivalence

  IM3FuncType
      retFuncTypes[c_m3Type_unknown]; // these 'point' to elements in the linked
                                      // list above. the number of elements must
                                      // match the basic types as per
                                      // M3ValueType
