...
/* WASM capability permissions - temporarily defined here until moved to kernel headers */
...
int sys_wasm_compile(Fcall *tx, Fcall *rx) {
    ...
    if (up->wasm.initialized) {
        rx->type = Rerror;
        snprint(rx->ename, sizeof(rx->ename), "WASM already compiled for this process");
        wasm_runtime.stats.errors++;
        return -1;
    }
    ...
    M3Result result = m3_ParseModule(wasm_runtime.env, (IM3Module*)&up->wasm.module,
    ...
    result = m3_LoadModule((IM3Runtime)up->wasm.runtime, (IM3Module)up->wasm.module);
    if (result) {
        m3_FreeRuntime((IM3Runtime)up->wasm.runtime);
        up->wasm.runtime = nil;
        up->wasm.module = nil;
    ...
    print("wasm_runtime: compiled pid=%lu memory=%u bytes (%u pages)\n",
          up->pid, up->wasm.memory_size, up->wasm.memory_pages);
...
int sys_wasm_execute(Fcall *tx, Fcall *rx) {
    ...
    IM3Function func;
    M3Result result = m3_FindFunction(&func, (IM3Runtime)up->wasm.runtime, func_name_buf);
    if (result) {
        rx->type = Rerror;
 ...
 * NOTE: In final architecture, process exit (pexit()) will handle this automatically.
 * This syscall allows explicit cleanup before process termination.
 */
