...
/*@
  ...
  @ assigns s1[\strlen(\old(s1)) .. \strlen(\old(s1)) + \min(n, \strlen(s2)) +
  1];
  @ ensures \result == s1;
  ...
  @ disjoint behaviors;
  @*/
char *strncat(char *s1, const char *s2, ulong n) {
  char *os1;
  ...
  /*@
    ...
    @ loop assigns s1, s2, n, os1[\strlen(\at(os1, Pre)) .. \strlen(\at(os1,
    Pre)) + \at(n, Pre)];
    @*/
  while (*s1++ = *s2++)
    if (--n < 0) {
