...
/*@
  ...
  @*/
static int isdelim(int c, char *sep) { return strchr(sep, c) != nil; }
...
/*@
  ...
  @             \valid_read(fields[i] + (0..ACSL_MAXSTR-1)) && fields[i] >= str;
  @ complete behaviors;
  @ disjoint behaviors;
  @*/
int getfields(char *str, char **fields, int n, int skip, char *sep) {
  char *s, *t;
  int nf;
