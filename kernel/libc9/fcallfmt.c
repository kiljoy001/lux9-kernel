...
int fcallfmt(Fmt *fmt) {
  ...
  switch (type) {
  case Tversion: /* 100 */
    seprint(buf, e, "Tversion tag %ud msize %ud version '%s'", tag, f->msize,
            f->version);
    break;
  case Rversion:
    seprint(buf, e, "Rversion tag %ud msize %ud version '%s'", tag, f->msize,
            f->version);
    break;
  case Tauth: /* 102 */
    seprint(buf, e, "Tauth tag %ud afid %d uname %s aname %s", tag, f->afid,
            f->uname, f->aname);
    break;
  case Rauth:
  ...
  case Rflush:
    ...
    break;
  case Twalk: /* 110 */
    p = seprint(buf, e, "Twalk tag %ud fid %d newfid %d nwname %d ", tag, fid,
                f->newfid, f->nwname);
  ...
  case Rwalk:
    ...
    if (f->nwqid <= MAXWELEM)
      for (i = 0; i < f->nwqid; i++) {
        q = &f->wqid[i];
        p = seprint(p, e, "%d:" QIDFMT " ", i, q->path, q->vers,
                    qidtype(tmp, q->type));
      }
    break;
  case Topen: /* 112 */
  ...
  case Ropen:
    ...
    break;
  case Tcreate: /* 114 */
    seprint(buf, e, "Tcreate tag %ud fid %ud name %s perm 0%luo mode %d", tag,
            fid, f->name, (ulong)f->perm, f->mode);
    break;
  case Rcreate:
    ...
    break;
  case Tread: /* 116 */
    seprint(buf, e, "Tread tag %ud fid %d offset %lld count %ud", tag, fid,
            f->offset, f->count);
    break;
  case Rread:
    ...
    break;
  case Twrite: /* 118 */
    p = seprint(buf, e, "Twrite tag %ud fid %d offset %lld count %ud ", tag,
  ...
  case Tsyscall:
    seprint(buf, e, "Tsyscall tag %ud scallnr %ud scount %ud", tag, f->scallnr,
            f->scount);
    break;
  case Rsyscall:
...
static char *qidtype(char *s, uchar t) {
  char *p;
...
static void fdirconv(char *buf, char *e, Dir *d) {
  ...
  seprint(buf, e,
          ...
          qidtype(tmp, d->qid.type), d->mode, d->atime, d->mtime, d->length,
          d->type, d->dev);
}
