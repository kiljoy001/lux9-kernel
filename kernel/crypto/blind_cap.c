...
/*@
  @ requires message != \null && \valid(message + (0..79));
  @ requires tok != \null && \valid_read(tok);
  @ assigns message[0..79];
  @*/
static void cap_serialize_token_message(u8int message[80], const CapToken *tok) {
...
int cap_blind_prepare(CapBlindRequest *req, const CapToken *tok) {
  /*
   * The current wire format signs the serialized token message directly.
   * The separate blinding factor is retained so the request format stays
   * extensible when a stronger blind-signature protocol is introduced.
   */
  cap_serialize_token_message(req->blinded_data, tok);
...
int cap_blind_sign(CapBlindResponse *resp, const CapBlindRequest *req,
  ...
  /* Sign the serialized token payload carried by the request. */
  crypto_eddsa_sign(resp->blind_signature, signing_key, req->blinded_data, 80);
  return 0;
}
...
int cap_token_verify(const CapToken *tok, const u8int *service_hash,
  ...
  cap_serialize_token_message(message, tok);
...
int cap_derive_epoch_keys(CapEpochKeys *out, const CapMasterKey *master,
  ...
  /* Generate Ed25519 keypair from a scratch copy: Monocypher wipes the seed. */
  memcpy(seed_copy, out->signing_seed, sizeof(seed_copy));
  crypto_eddsa_key_pair(secret_key, out->signing_pubkey, seed_copy);
