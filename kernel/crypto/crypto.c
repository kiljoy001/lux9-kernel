...
extern void sha256_transform_hw(uint32_t state[8], const uint8_t block[64],
                                uint32_t nblocks);
 ...
 */
static int crypto_sha_hw_disabled(void) {
  static int checked;
  static int disabled;
  char *p;
  ...
  if ((p = getconf("sha")) != nil && *p == '0')
    disabled = 1;
  if (getconf("nohwsha") != nil)
    disabled = 1;
  if (getconf("*nohwsha") != nil)
    disabled = 1;
...
int crypto_hw_sha_available(void) {
  ...
  if (crypto_sha_hw_disabled())
    return 0;
  return m->havesha && fpu_ready;
}
 ...
 */
int crypto_hw_rdrand_available(void) {
  /* Access current CPU's Mach structure */
  extern Mach *m;
  return m->haverdrand;
}
...
int crypto_sha256(uint8_t *out, const uint8_t *data, size_t len) {
  ...
  if (crypto_hw_sha_available() && len >= 64) {
    ...
    if (remainder > 0 || len == 0) {
      ...
      ctx.count = (uint64_t)full_blocks * 64;
...
int crypto_tpm_key_init(void) {
  ...
  if (ret == CRYPTO_HMAC_KEY_BYTES) {
    ...
    if (crypto_hw_rdrand_available()) {
      ...
      print("Crypto: System requires hardware TPM or RDRAND for cryptographic "
            "operations\n");
  ...
  if (tpm_key_state.key_valid) {
    int seal_ret =
        crypto_tpm_seal_key(tpm_key_state.hmac_key, CRYPTO_HMAC_KEY_BYTES);
    if (seal_ret == 0) {
...
int crypto_tpm_seal_key(const uint8_t *key, size_t keylen) {
  ...
  ret = tpm2_create(tpm_sealed_key.srk_handle, (u8int *)key, (u16int)keylen,
...
int crypto_tpm_unseal_key(uint8_t *key_out, size_t *keylen) {
  ...
  ret = tpm2_load(tpm_sealed_key.srk_handle, tpm_sealed_key.private_blob,
                  tpm_sealed_key.private_len, tpm_sealed_key.public_blob,
                  tpm_sealed_key.public_len, &obj_handle);
