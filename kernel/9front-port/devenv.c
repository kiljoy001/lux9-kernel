 ...
 *   - ref >= 1 at all times (Inv_RefPositive)
 *   - alloc <= Maxenvsize (Inv_AllocBounded)
...
static Evalue *envindex(Egrp *eg, uvlong qidpath) {
  Evalue *e;
  int i;
...
static Evalue **envhash(Egrp *eg, char *name) {
  uint c, h = 0;
  while (c = *name++)
    h = h * 131 + c;
...
static Evalue *lookupname(Evalue *e, char *name) {
  while (e != nil) {
...
static int envgen(Chan *c, char *name, Dirtab *, int, int s, Dir *dp) {
  Egrp *eg;
  Evalue *e;
  Qid q;
...
/**
 * Attach to the env device, optionally selecting the global configuration
 * group.
 *
 ...
 *
 * @param spec Optional device specifier; "c" selects the global configuration
 * group, nil or empty string selects the default (per-process) environment.
 * @returns Pointer to a newly attached Chan with its aux field set to the
 * selected Egrp
 */
static Chan *envattach(char *spec) {
  Chan *c;
  Egrp *egrp = nil;
 ...
 * @param nc Optional channel to update with the final walked state (may be
 * nil).
 * @param name Array of path element strings to consume during the walk.
 * @param nname Number of elements in `name`.
 * @returns Pointer to a Walkqid list describing the walked path on success,
 * `nil` on failure.
 */
static Walkqid *envwalk(Chan *c, Chan *nc, char **name, int nname) {
  return devwalk(c, nc, name, nname, nil, 0, envgen);
}

static int envstat(Chan *c, uchar *db, int n) {
  return devstat(c, db, n, nil, 0, envgen);
}

static void *envrealloc(Egrp *eg, void *old, int newsize) {
  ...
  if (newsize < 0 ||
      eg != &confegrp && (eg->alloc + newsize) - oldsize > Maxenvsize)
    error(Enomem);
...
static Chan *envopen(Chan *c, int omode) {
  Egrp *eg;
  Evalue *e;
...
Dev envdevtab = {
    ...
    devreset, envinit,  devshutdown, envattach, envwalk,
...
/*@ requires eg != \null;
    ...
    behavior confegrp:
      assumes eg == &confegrp;
      assigns \nothing;
    // COQ_PROOF_REF: proofs/env/conservation.v:decref_decrements
    // COQ_PROOF_REF: proofs/env/conservation.v:closeegrp_frees_on_last_ref
*/
void closeegrp(Egrp *eg) {
  Evalue *e;
  int i;
 ...
 */
void ksetenv(char *ename, char *eval, int conf) {
  Chan *c;
  char buf[2 * KNAMELEN];
