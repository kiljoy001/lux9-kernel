...
pci_framework_init(void)
{
    ...
    pci_framework.pci_devices = malloc(sizeof(Device*) * pci_framework.pci_device_capacity);
    if (pci_framework.pci_devices == nil) {
        pci_framework.pci_device_capacity = 0;
    }
    
    if (0) print("pci_framework: initialized\n");
}
...
{
    for (int i = 0; pci_classes[i].base_class != 0xFF || pci_classes[i].sub_class != 0xFF; i++) {
        if (pci_classes[i].base_class == base_class && pci_classes[i].sub_class == sub_class) {
...
pci_framework_register_device(Pcidev *pcidev)
{
    ...
    if (driver != nil) {
        ...
        snprint(dev->description, sizeof(dev->description),
                "%s %s (%04x:%04x)", 
                class_info->class_name, class_info->sub_class_name,
                pcidev->vid, pcidev->did);
        
        /* Register driver if probe succeeds */
        if (driver->probe == nil || driver->probe(pcidev) == 0) {
            devregistry_register_driver(dev, driver->name, 0); /* PID 0 for kernel drivers */
    ...
    if (pci_framework.pci_device_count >= pci_framework.pci_device_capacity) {
        ...
        Device **new_array = realloc(pci_framework.pci_devices, sizeof(Device*) * new_capacity);
        if (new_array != nil) {
...
pci_framework_enumerate(void)
{
    ...
    if (0) print("pci_framework: enumerating PCI devices\n");
    ...
    if (0) print("pci_framework: found %d PCI devices\n", count);
...
pci_framework_register_driver(PCIDriver *driver)
{
    ...
    if (0) print("pci_framework: registered driver %s\n", driver->name);
...
pci_framework_unregister_driver(PCIDriver *driver)
{
    ...
    if (0) print("pci_framework: unregistered driver %s\n", driver->name);
...
pci_framework_list_drivers(void (*print_func)(char*, ...))
{
    ...
    for (driver = pci_framework.drivers; driver != nil; driver = driver->next) {
        print_func("%s: VID:%04x DID:%04x Class:%02x:%02x\n",
                   driver->name,
                   driver->vendor_id, driver->device_id,
                   driver->base_class, driver->sub_class);
    }
    unlock(&pci_framework.lock);
...
pci_framework_list_devices(void (*print_func)(char*, ...))
{
    ...
    for (int i = 0; i < pci_framework.pci_device_count; i++) {
        ...
        if (dev != nil) {
            PCIClass *class_info = pci_get_class_info(
                dev->location.pci.pcidev->ccrb, 
