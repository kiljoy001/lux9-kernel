...
static void tpminit_dev(void) {
  /* Initialize TPM hardware */
  tpminit();
}

static Chan *tpmattach(char *spec) { return devattach('Ϯ', spec); }

static Walkqid *tpmwalk(Chan *c, Chan *nc, char **name, int nname) {
  return devwalk(c, nc, name, nname, tpmdir, nelem(tpmdir), devgen);
}

static long tpmstat(Chan *c, uchar *db, long n) {
  return devstat(c, db, n, tpmdir, nelem(tpmdir), devgen);
}

static Chan *tpmopen(Chan *c, int omode) {
  /* Validate access */
...
static void tpmclose(Chan *c) { USED(c); }
...
*/
static long tpmread(Chan *c, void *va, long n, vlong off) {
  u8int buf[512];
  usize len;
  int nr, pcr;
...
/*@ requires c != \null;
    ...
    assigns \nothing;
*/
static long tpmwrite(Chan *c, void *va, long n, vlong off) {
  u8int hash[32];
  int pcr;
  Cmdbuf *cb;
