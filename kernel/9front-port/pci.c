...
vlong pcibarsize(Pcidev *p, int rno) {
  ...
  if (size < 0 || (size & (size - 1)) != 0) {
    print("pcibarsize: %ux invalid bar rno %#x size %#llux mask %#llux\n",
...
static int pcivalidwin(Pcidev *p, uvlong base, uvlong limit) {
  ...
  if (base & 1) {
    ...
    if (bridge == nil)
      return 1;
    if (base >= bridge->mema.bar &&
        limit < (bridge->mema.bar + bridge->mema.size))
      return 1;
    if (base >= bridge->prefa.bar &&
        limit < (bridge->prefa.bar + bridge->prefa.size))
      return 1;
  }
  print("%ux: %.2uX invalid %s-window: %.8llux-%.8llux\n", p->tbdf, p->ccrb,
        typ, base, limit);
  return 0;
}
...
Pcidev *pcimatchtbdf(int tbdf) {
  ...
  for (pcidev = pcilist; pcidev != nil; pcidev = pcidev->list) {
    if (++n > 1000) {
      print("pcimatchtbdf: CYCLE DETECTED in pcilist! tbdf=%ux\n", tbdf);
      return nil;
    }
    if (pcidev->tbdf == tbdf)
      break;
...
int pcicap(Pcidev *p, int cap) { return pcienumcaps(p, matchcap, cap); }

int pcihtcap(Pcidev *p, int cap) { return pcienumcaps(p, matchhtcap, cap); }
