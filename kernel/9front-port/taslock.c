...
static void lockloop(Lock *l, uintptr pc) {
  Proc *p;
    ...
    while (l->key) {
      if (conf.nmach < 2 && up && up->edf && (up->edf->flags & Admitted)) {
        ...
        print("inversion %#p pc %#p proc %lud held by pc %#p proc %lud\n", l,
              pc, up ? up->pid : 0, l->pc, l->p ? l->p->pid : 0);
...
void ilock(Lock *l) {
  ...
  if (tas(&l->key) != 0) {
    ...
    for (;;) {
      ...
      while (l->key) {
        if (l->p == up) {
          print("ilock: RECURSIVE LOCK DETECTED! lock=%#p holder=%#p "
                "caller=%#p\n",
                l, l->pc, pc);
          panic("ilock: recursive lock");
        }
        if (i++ > 10000000) {
          i = 0;
          print("ilock spin lock=%#p holder_pc=%#p holder_pid=%lud caller=%#p "
                "caller_pid=%lud\n",
                l, l->pc, l->p ? l->p->pid : 0, pc, up ? up->pid : 0);
        }
      }
      x = splhi();
      if (tas(&l->key) == 0)
...
void unlock(Lock *l) {
  ...
  if (l->isilock)
    print("unlock(%#p) of ilock: pc %#p, held by %#p\n", l, getcallerpc(&l),
          l->pc);
  if (l->p != up) {
    print("unlock(%#p): up changed: pc %#p, acquired at pc %#p, lock p %#p, "
          "unlock up %#p\n",
