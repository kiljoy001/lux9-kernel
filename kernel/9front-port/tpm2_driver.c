...
static int tpm_wait_status(uintptr reg_offset, u8int mask, u8int expected,
                           int timeout_ms) {
  int i;
  u8int status;
...
static int tpm_request_locality(void) {
  ...
  /* Wait for locality */
  if (tpm_wait_status(TPM_ACCESS_0, TPM_ACCESS_ACTIVE_LOCALITY,
                      TPM_ACCESS_ACTIVE_LOCALITY, 1000) < 0) {
    access = tpm_read8(TPM_ACCESS_0);
 ...
 */
int tpm_transmit(TPMContext *ctx, u8int *cmd, usize cmd_len, u8int *resp,
                 usize *resp_len) {
  USED(ctx);
  int i, burst, count;
  u8int status;
  ...
  if (!(status & TPM_STS_COMMAND_READY)) {
    ...
    if (tpm_wait_status(TPM_STS_0, TPM_STS_COMMAND_READY, TPM_STS_COMMAND_READY,
                        1000) < 0) {
...
void tpminit(void) {
  ...
  uintptr
      virt_base; /* Use uintptr instead of void* to avoid pointer truncation */
  ulong tpm_size;
  ...
  tpm_size =
      PGROUND(16 * 1024); /* Map 16KB to cover all localities, page-aligned */
 ...
 *
 * IMPORTANT: TPM I/O requires interrupts for completion.
 * If interrupts are disabled (inside ilock, during poolalloc, etc.),
...
int tpm_get_random(u8int *buffer, int len) {
  ...
  /*
   * CRITICAL: Check if interrupts are enabled.
   * islo() returns non-zero if interrupts are enabled.
   * TPM I/O waits for hardware completion via polling which
   * internally depends on timer interrupts for timeout handling.
   * If called with interrupts disabled, we would deadlock.
   */
  if (!islo())
    return -1;
