...
static void
secure_wipe(uchar *data, ulong size)
{
	ulong i;
...
static int
derive_key_from_password(const char *password, uchar *salt, uchar *key_out)
{
...
static void
xchacha20_crypt(uchar *data, ulong size, uchar *key, uchar *nonce)
{
	uint64_t ctr = 0;
...
ramclose(Chan *c)
{
	...
	if((ulong)c->qid.path == Qsecureram && secure_rd.locked && secure_rd.data != nil) {
...
ramread(Chan *c, void *va, long n, vlong off)
{
	...
	case Qsecureramctl:
		...
		snprint(status, sizeof(status), "status: %s\nsize: %lud\n",
				secure_rd.locked ? "locked" : "unlocked",
				secure_rd.size);
...
ramwrite(Chan *c, void *va, long n, vlong off)
{
	...
	case Qsecureramctl:
		...
		if(strcmp(argv[0], "init") == 0) {
			...
			if(derive_key_from_password(argv[1], secure_rd.salt, secure_rd.master_key) < 0)
