...
void devregistry_init(void) {
  ...
  if (0)
    print("devregistry: initialized\n");
}

/* Generate unique device ID */
static ulong devregistry_generate_id(void) { return devregistry.next_id++; }
...
/*@
  @ requires pcidev != \null;
  @ ensures \result == \null || \valid(\result);
  @ assigns devregistry.devices, devregistry.device_count,
  @         devregistry.type_count[DEVTYPE_PCI],
  @         devregistry.state_count[DEVSTATE_PRESENT];
  @*/
Device *devregistry_register_pci(Pcidev *pcidev) {
  ...
  /* Create device name */
  snprint(dev->name, sizeof(dev->name), "%04x:%02x:%02x.%d", 0,
          dev->location.pci.busno, dev->location.pci.devno,
          dev->location.pci.funcno);

  /* Create description */
  snprint(dev->description, sizeof(dev->description), "PCI device %04x:%04x",
  ...
  if (0)
    print("devregistry: registered PCI device %s (VID:%04x DID:%04x)\n",
...
/*@
  @ ensures \result == \null || \valid(\result);
  @ assigns devregistry.devices, devregistry.device_count,
  @         devregistry.type_count[DEVTYPE_USB],
  @         devregistry.state_count[DEVSTATE_PRESENT];
  @*/
Device *devregistry_register_usb(int busno, int devno, int funcno, ulong vid,
                                 ulong pid) {
  ...
  /* Create device name */
  snprint(dev->name, sizeof(dev->name), "usb-%d-%d.%d", busno, devno, funcno);

  /* Create description */
  snprint(dev->description, sizeof(dev->description), "USB device %04x:%04x",
          vid, pid);
  ...
  if (0)
    print("devregistry: registered USB device %s (VID:%04x PID:%04x)\n",
          dev->name, vid, pid);
...
/* Register a platform device */
/*@
  @ requires name == \null || valid_string_or_null(name);
  @ ensures \result == \null || \valid(\result);
  @ assigns devregistry.devices, devregistry.device_count,
  @         devregistry.type_count[DEVTYPE_PLATFORM],
  @         devregistry.state_count[DEVSTATE_PRESENT];
  @*/
Device *devregistry_register_platform(ulong phys_addr, ulong size, ulong vid,
                                      ulong pid, char *name) {
  ...
  if (0)
    print("devregistry: registered platform device %s (VID:%04x PID:%04x)\n",
          dev->name, vid, pid);
...
Device *devregistry_find_by_pci(int busno, int devno, int funcno) {
  ...
  lock(&devregistry.lock);
  for (dev = devregistry.devices; dev != nil; dev = dev->next) {
    if (dev->type == DEVTYPE_PCI && dev->location.pci.busno == busno &&
...
int devregistry_update_state(Device *dev, DevState new_state) {
  ...
  if (0)
    print("devregistry: device %s state changed to %d\n", dev->name, new_state);
...
void devregistry_list(void (*print_func)(char *, ...)) {
  ...
  char *state_names[] = {"Unknown", "Present", "Configured",
...
@*/
  for (int i = 0; i < DEVTYPE_MAX; i++) {
    if (devregistry.type_count[i] > 0) {
      print_func("%s=%d ", i < DEVTYPE_MAX ? type_names[i] : "Invalid",
...
@*/
  for (int i = 0; i < DEVSTATE_MAX; i++) {
    if (devregistry.state_count[i] > 0) {
      print_func("%s=%d ", i < DEVSTATE_MAX ? state_names[i] : "Invalid",
...
int devregistry_register_driver(Device *dev, char *driver_name,
                                int driver_pid) {
  ...
  if (0)
    print("devregistry: registered driver %s for device %s\n", driver_name,
          dev->name);
...
int devregistry_unregister_driver(Device *dev) {
  ...
  if (0)
    print("devregistry: unregistered driver from device %s\n", dev->name);
