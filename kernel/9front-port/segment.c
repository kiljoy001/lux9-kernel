...
Segment *newseg(int type, uintptr base, ulong size) {
  Segment *s;
  int mapsize;
...
void putseg(Segment *s) {
  ...
  if (s->mapsize > 0) {
    print("putseg: s=%p type=%#x map=%p mapsize=%d map0=%p\n", s, s->type,
          s->map, s->mapsize, s->map != nil ? s->map[0] : nil);
    Pte **pte, **emap;
    Page *fh, *ft;
    ulong np;
...
void segpage(Segment *s, Page *p) {
  ...
  qlock(&s->qlock);
  print("segpage: process '%s' pid %d calling for p=%p va=%#p pa=%#p\n",
        up ? up->text : "nil", up ? up->pid : -1, p, p->va, p->pa);
...
Segment *isoverlap(uintptr va, uintptr len) {
  int i;
  Segment *ns;
  uintptr newtop;
...
Physseg *addphysseg(Physseg *new) {
  Physseg *ps, *existing;
