...
tpm_test_run(void)
{
    ...
    uint32_t nv_index = 0x01800001; /* Test NV Index */
    ...
    ret = tpm2_create_primary(0, &srk_handle);
    if(ret == 0){
    ...
    /* 7. NVRAM Undefine (for clean state) */
    print("Test 7: TPM2_NV_UndefineSpace (Index 0x%08X - cleanup)... ", nv_index);
    tpm2_nv_undefine_space(0, nv_index); // Ignore error if not found
    print("DONE\n");

    /* 8. NVRAM Define */
    print("Test 8: TPM2_NV_DefineSpace (Index 0x%08X)... ", nv_index);
    /* Attributes: TPMA_NV_OWNERWRITE | TPMA_NV_OWNERREAD | TPMA_NV_AUTHREAD | TPMA_NV_WRITEDEFINE | TPMA_NV_WRITE_ST_CLEAR */
    ret = tpm2_nv_define_space(0, nv_index, 32, 0x20000000 | 0x00020000 | 0x00040000 | 0x00000004 | 0x00000002);
    if(ret == 0){
        print("OK\n");
    } else {
        print("FAIL (error 0x%08X)\n", (u32int)ret);
    }

    /* 9. NVRAM Write */
    print("Test 9: TPM2_NV_Write... ");
    ret = tpm2_nv_write(0, nv_index, (u8int*)"PersistentData", 14, 0);
    print("%s\n", ret == 0 ? "OK" : "FAIL");

    /* 10. NVRAM Undefine */
    print("Test 10: TPM2_NV_UndefineSpace... ");
