 ...
 * Called by print() in libc9/print.c
 */
void
screenputs(char *s, int n)
{
    ...
    if(kmesg.n + n > sizeof(kmesg.buf)){
        ...
        int avail = sizeof(kmesg.buf) - kmesg.n;
        if(avail > 0) {
            memmove(kmesg.buf + kmesg.n, s, avail);
            kmesg.n += avail;
        }
    } else {
        memmove(kmesg.buf + kmesg.n, s, n);
        kmesg.n += n;
    }
    iunlock(&kmesg.lk);
}
...
panic(char *fmt, ...)
{
    ...
    print("panic: %s\n", buf);
    /* In a real deployment, dump stack here */
