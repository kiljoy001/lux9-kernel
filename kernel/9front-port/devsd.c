...
static int sdmountdrivers(void) {
  Chan *c;
...
static void sdunmountdrivers(void) {
  if (ahci_chan != nil) {
    cclose(ahci_chan);
    ahci_chan = nil;
  }
...
static long sd9pread(Chan *mntchan, void *buf, long n, vlong offset) {
  return devtab[mntchan->type]->read(mntchan, buf, n, offset);
}

static long sd9pwrite(Chan *mntchan, void *buf, long n, vlong offset) {
  return devtab[mntchan->type]->write(mntchan, buf, n, offset);
}

static void sdprobe(void) {
...
static void sdinit(void) { /* Nothing - probing happens during reset */ }

static Chan *sdattach(char *spec) { return devattach('S', spec); }
...
static void sdclose(Chan *c) { /* Nothing to do for block device close */ }
...
static long sdwrite(Chan *c, void *a, long n, vlong offset) {
  ...
  case Qsdbase:
    ...
    /* Allocate buffers */
    buf = smalloc_driver(count * 512);
    tempbuf = smalloc_driver(512);
    if (buf == nil || tempbuf == nil) {
      if (buf)
        xfree_driver(buf);
      if (tempbuf)
        xfree_driver(tempbuf);
      error(Enomem);
    }
...
static Block *sdbread(Chan *c, long n, ulong offset) {
  Block *bp;
  long nbytes;
...
static long sdbwrite(Chan *c, Block *bp, ulong offset) {
  long n;
