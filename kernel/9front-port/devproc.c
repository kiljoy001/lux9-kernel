...
static Cmdtab proccmd[] = {
    CMclose,        "close",        2, CMclosefiles,    "closefiles",    1,
    CMfixedpri,     "fixedpri",     2, CMhang,          "hang",          1,
    CMnohang,       "nohang",       1, CMnoswap,        "noswap",        1,
    CMkill,         "kill",         1, CMpri,           "pri",           2,
    CMprivate,      "private",      1, CMprofile,       "profile",       1,
    CMstart,        "start",        1, CMstartstop,     "startstop",     1,
    CMstartsyscall, "startsyscall", 1, CMstop,          "stop",          1,
    CMwaitstop,     "waitstop",     1, CMwired,         "wired",         2,
    CMtrace,        "trace",        0, CMinterrupt,     "interrupt",     1,
    CMnointerrupt,  "nointerrupt",  1, CMperiod,        "period",        2,
    CMdeadline,     "deadline",     2, CMcost,          "cost",          2,
    CMsporadic,     "sporadic",     1, CMdeadlinenotes, "deadlinenotes", 1,
    CMadmit,        "admit",        1, CMextra,         "extra",         1,
    CMexpel,        "expel",        1, CMevent,         "event",         1,
};

/* Segment type from portdat.h */
static char *sname[] = {"Text",   "Data", "Bss",   "Stack",
                        "Shared", "Phys", "Fixed", "Sticky"};
...
static Chan *procopen(Chan *c, int omode0) {
  Proc *p;
  Chan *tc;
  int pid;
  int omode;
...
int procfdprint(Chan *c, int fd, char *s, int ns) {
  return snprint(s, ns,
                 ...
                 &"r w rw"[(c->mode & 3) << 1], devtab[c->type] -> dc, c -> dev,
                 c -> qid.path, c->qid.vers, c->qid.type, c->iounit, c->offset,
                 c->path->s);
}
...
/*
 * setupwatchpts(Proc *p, Watchpt *wp, int nwp) is defined for all arches
 * separately. It tests whether wp is a valid set of watchpoints and errors out
 * otherwise. If and only if they are valid, it sets up all watchpoints
 * (clearing any preexisting ones). This is to make sure that failed writes to
...
static int readwatchpt(Proc *pr, char *buf, int nbuf) {
  ...
  for (w = pr->watchpt; w < pr->watchpt + pr->nwatchpt; w++)
    p = seprint(p, e,
                sizeof(uintptr) == 8 ? "%c%c%c %#.16p %#.16p\n"
                                     : "%c%c%c %#.8p %#.8p\n",
                (w->type & WATCHRD) != 0 ? 'r' : '-',
                (w->type & WATCHWR) != 0 ? 'w' : '-',
                (w->type & WATCHEX) != 0 ? 'x' : '-', (void *)w->addr,
                (void *)w->len);
  return p - buf;
}
...
static int writewatchpt(Proc *pr, char *buf, int nbuf, uvlong offset) {
  ...
  for (wq = wp + nwp0; wq < wp + nwp0 + nwp;) {
    ...
    for (q = f[0]; *q != 0; q++)
      switch (*q) {
      case 'r':
        if ((wq->type & WATCHRD) != 0)
          goto tinval;
        wq->type |= WATCHRD;
        break;
      case 'w':
        if ((wq->type & WATCHWR) != 0)
          goto tinval;
        wq->type |= WATCHWR;
        break;
      case 'x':
        if ((wq->type & WATCHEX) != 0)
          goto tinval;
        wq->type |= WATCHEX;
        break;
      case '-':
    ...
    x = strtoull(f[1], &q, 0);
    if (f[1] == q || *q != 0 || x != (uintptr)x)
      error("invalid address");
    wq->addr = x;
    x = strtoull(f[2], &q, 0);
    if (f[2] == q || *q != 0 || x > (uintptr)-wq->addr)
      error("invalid length");
    wq->len = x;
    if (wq->addr + wq->len > USTKTOP)
      error("bad address");
    wq++;
  }
  nwp = wq - (wp + nwp0);
...
static long procread(Chan *c, void *va, long n, vlong off) {
  ...
  if (QID(c->qid) == Qtrace) {
    ...
    while (navail > 0) {
      ne = ((tconsumed & Emask) + navail > Nevents)
               ? Nevents - (tconsumed & Emask)
               : navail;
      memmove(rptr, &tevents[tconsumed & Emask], ne * sizeof(Traceevent));
  ...
  case Qpid2:
    if (offset >= sizeof(p->pid2.data))
      return 0;
    if (offset + n > sizeof(p->pid2.data))
      n = sizeof(p->pid2.data) - offset;
    memmove(va, p->pid2.data + offset, n);
    return n;
  ...
  case Qstatus:
    ...
    j = snprint(
        ...
        "%11lud %11lud %11lud "
        "%11lud %11lud %11lud\n",
        p->text, p->user, sps, tk2ms(p->time[TUser]), tk2ms(p->time[TSys]),
        tk2ms(MACHP(0)->ticks - p->time[TReal]), tk2ms(p->time[TCUser]),
        tk2ms(p->time[TCSys]), tk2ms(p->time[TCReal]),
        (ulong)(procpagecount(p) * BY2PG / 1024), p->basepri, p->priority);
  statbufread:
    if (offset >= j)
      return 0;
    if (offset + n > j)
  ...
  case Qsegment:
    ...
    for (i = 0; i < NSEG; i++) {
      ...
      if (s == nil)
        continue;
      j += snprint(statbuf + j, sizeof(statbuf) - j, "%-6s %c%c %8p %8p %4ld\n",
                   sname[s->type & SG_TYPE],
                   s->type & SG_FAULT ? 'F' : (s->type & SG_RONLY ? 'R' : ' '),
                   s->profile ? 'P' : ' ', s->base, s->top, s->ref);
    }
    goto statbufread;
  ...
  case Qwait:
    ...
    j = snprint(statbuf, sizeof(statbuf), "%d %lud %lud %lud %q", wq->w.pid,
...
static Chan *proctext(Chan *c, Proc *p) {
  Chan *tc;
  Image *i;
  Segment *s;
...
static char *parsetime(vlong *rt, char *s) {
  uvlong ticks;
  ulong l;
  char *e, *p;
...
static int procstopped(void *a) { return ((Proc *)a)->state == Stopped; }
...
static long procctlmemio(Chan *c, Proc *p, uintptr offset, void *a, long n,
  ...
  print("procctlmemio: ENTER pid %d (target pid %d) offset %p n %ld read %d\n",
        up->pid, p->pid, offset, n, read);
  ...
  if (!read && (s->type & SG_TYPE) == SG_TEXT) {
    print("procctlmemio: calling txt2data for pid %d (target pid %d)\n",
          up->pid, p->pid);
    p->seg[i] = txt2data(s);
    qunlock(&s->qlock);
    putseg(s);
