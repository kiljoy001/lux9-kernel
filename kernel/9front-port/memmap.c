...
static void dump1(Mapent *e) {
  print("%.16llux-%.16llux %lux\n", e->addr, e->addr + e->size, e->type);
}

static int insert(uvlong addr, uvlong size, ulong type) {
  Mapent *e;
...
static Mapent *lookup(uvlong addr) {
  Mapent *i, *e;
...
static void sort(void) {
  ...
  uartprintf("CHECK: sort entry n=%d m=%d\n", mapalloc.n, mapalloc.m);

Again:
  if (mapalloc.m == 0)
    return;
  uartprintf("CHECK: sort merging n=%d m=%d\n", mapalloc.n, mapalloc.m);
  mapalloc.n += mapalloc.m;
  mapalloc.m = 0;

  /* Use standard qsort now that it is implemented */
  qsort(mapalloc.a, mapalloc.n, sizeof(*e), compare);
  // uartprintf("CHECK: qsort returned\n");
  ...
  while (i < e) {
    ...
    for (j = i + 1; j < e; j++) {
      ...
      if (j->addr - i->addr + j->size < i->size)
        if (!insert(j->addr + j->size, i->size - (j->addr + j->size - i->addr),
                    i->type))
          continue;
...
uvlong memmapnext(uvlong addr, ulong type) {
  Mapent *i, *e;
  extern void uartprintf(char *, ...); // Declaration

  uartprintf("CHECK: memmapnext(addr=%llx, type=%lx) key=%d\n", addr, type,
             mapalloc.lk.key);
  lock(&mapalloc.lk);
  uartprintf("CHECK: memmapnext locked\n");
  sort();
  uartprintf("CHECK: memmapnext sorted\n");
  for (i = mapalloc.a, e = i + mapalloc.n; i < e; i++) {
    if (((i->type ^ type) & ~Allocated) == 0 &&
        (addr == -1 || i->addr > addr)) {
      addr = i->addr;
      uartprintf("CHECK: memmapnext found addr=%llx\n", addr);
      unlock(&mapalloc.lk);
      return addr;
    }
  }
...
uvlong memmapsize(uvlong addr, uvlong align) {
  Mapent *i;
  uvlong size;
...
void memmapadd(uvlong addr, uvlong size, ulong type) {
  int result;
  type &= ~Allocated;
...
uvlong memmapalloc(uvlong addr, uvlong size, uvlong align, ulong type) {
  Mapent *i, *e;
...
void memmapfree(uvlong addr, uvlong size, ulong type) {
  Mapent *i;

  lock(&mapalloc.lk);
  sort();
  i = lookup(addr);
  if (i == nil || i->type != (type | Allocated) ||
