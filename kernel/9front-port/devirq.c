 ...
 */
static int irqgen(Chan *c, char *name, Dirtab *tab, int ntab, int pos,
                  Dir *dp) {
  Qid qid;
  char buf[32];
  int irq;
...
static Chan *irqattach(char *spec) { return devattach('I', spec); }

static Walkqid *irqwalk(Chan *c, Chan *nc, char **name, int nname) {
...
static Chan *irqopen(Chan *c, int omode) {
  int irq;
  ...
  case Qctl:
    ...
    for (i = 0; i < MaxIRQ; i++) {
      if (irqstate[i].registered) {
        ...
        int entry_len = snprint(
            nil, 0, "irq %d: %s pending=%ud delivered=%ud dropped=%ud\n", i,
            irqstate[i].name, irqstate[i].pending, irqstate[i].delivered,
            irqstate[i].dropped);
        ...
        len += snprint(buf + len, sizeof(buf) - len,
                       ...
                       irqstate[i].name, irqstate[i].pending,
                       irqstate[i].delivered, irqstate[i].dropped);
      }
    }
...
static long irqwrite(Chan *c, void *va, long n, vlong off) {
  ...
  case Qctl:
    ...
    if (nf >= 3 && strcmp(fields[0], "register") == 0) {
      ...
      intrdisable(irq, irq_userspace_handler, is, BUSUNKNOWN, is->name);
