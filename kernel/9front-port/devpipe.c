...
pipeattach(char *spec)
{
	...
	if(p->q[0] == nil || p->q[1] == nil){
		if(p->q[0]) qfree(p->q[0]);
		if(p->q[1]) qfree(p->q[1]);
		free(p);
		error(Enomem);
	}
	p->qref[0] = 0;
...
static int
pipegen(Chan *c, char *name, Dirtab *tab, int ntab, int s, Dir *dp)
{
	int id;
	Qid q;
	Pipe *p;
			...
			*/
			...
			if(c->flag & COPEN){
				...
				if(c->mode == OREAD || c->mode == ORDWR){
					...
					// We read from q[1]. We should close q[1] so writers to data1 get broken pipe?
					qclose(p->q[1-id]);
				}
			}
		}
