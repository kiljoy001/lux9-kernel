...
char *statename[] = {
    ...
    "Dead",    "Moribund", "New",       "Ready",       "Scheding",
    "Running", "Queueing", "QueueingR", "QueueingW",   "Wakeme",
    "Broken",  "Stopped",  "Rendez",    "Waitrelease",
};
...
_Noreturn void schedinit(void) {
  ...
  if (up != nil) {
    ...
    case Moribund:
      mmurelease(up);
      lock(&procalloc);
      /* up->state = Dead; -- REPLACED BY FSM */
      proc_event(up, EV_REAP);
      up->mach = nil;
...
int kenter(Ureg *ureg) {
  ...
  if (user) {
    ...
    if (rem < 256) {
      panic("kenter: %d stack bytes left, up %#p ureg %#p at pc %#p", rem, up,
            ureg, ureg->pc);
    }
    kenterdebug++;
  }
...
void sched(void) {
  ...
  /* up->state = Running; -- REPLACED BY FSM */
  proc_event(up, EV_SCHEDULE);
  /* Critical section: disable interrupts during context switch */
  splhi();
  mmuswitch(up);
  gotolabel(&up->sched);
  /* Note: splx() not called here because gotolabel() never returns to this
   * point */
}

int anyready(void) { return runvec; }

int anyhigher(void) { return runvec & ~((1 << (up->priority + 1)) - 1); }
 ...
 */
static int reprioritize(Proc *p) {
...
static int queueproc(Schedq *rq, Proc *p) {
  ...
  /* p->state = Ready; -- REPLACED BY FSM */
  /* Infer event from current state */
  int event = EV_READY;
  switch (p->state) {
  case New:
    event = EV_READY;
  ...
  case Broken:
    event = EV_READY;
    break; /* Or EV_CONT/FIX? Using READY for now */
  case Stopped:
    event = EV_CONT;
    break;
  case Rendezvous:
...
Proc *dequeueproc(Schedq *rq, Proc *tp) {
  ...
  if (p->state != Ready) {
    iprint("dequeueproc %s %lud %s pc %p\n", p->text, p->pid,
           statename[p->state], getcallerpc(&rq));
    p = nil;
  }
  unlock(rq);
  return p;
...
Proc *runproc(void) {
  ...
  if ((p = m->readied) != nil && p->mach == nil && p->state == Ready &&
      (!p->wired || p->affinity == m->machno) && runq[Nrq - 1].head == nil &&
      runq[Nrq - 2].head == nil) {
    skipscheds++;
    rq = &runq[p->priority];
    goto found;
...
Proc *newproc(void) {
  ...
  p->psstate = nil;
  /* p->state = New; -- REPLACED BY FSM */
  proc_event(p, EV_CREATE);
  p->mach = nil; /* Ensure mach is nil for new process */
  __asm__ volatile("" ::
                       : "memory"); /* Compiler barrier to prevent reordering */
  p->fpstate = FPinit;
#ifdef KFPSTATE
  p->kfpstate = FPinit;
#endif
...
void procpriority(Proc *p, int pri, int fixed) {
  if (pri >= Npriq)
    pri = Npriq - 1;
  else if (pri < 0)
    pri = 0;
...
void procinit0(void) /* bad planning - clashes with devproc.c */
{
  procalloc.free = nil;
...
void sleep(Rendez *r, int (*f)(void *), void *arg) {
  ...
  if (up->nlocks)
    print("process %s %lud sleeps with %d locks held, "
          ...
          up->text, up->pid, up->nlocks, up->lastlock, up->lastlock->pc,
          getcallerpc(&r));
  ...
  lock(&up->rlock);
  if (r->p != nil) {
    print("sleep: double sleep r=%#p caller=%#p prev=%s[%lud] state=%s "
          "cur=%s[%lud]\n",
          r, getcallerpc(&r), r->p->text, r->p->pid, statename[r->p->state],
          up->text, up->pid);
    panic("double sleep r=%#p caller=%#p prev=%s[%lud] state=%s cur=%s[%lud]",
          r, getcallerpc(&r), r->p->text, r->p->pid, statename[r->p->state],
          up->text, up->pid);
  }
...
char *popnote(Ureg *u) {
  ...
  if (u != nil && up->lastnote->ref == 1 &&
      strncmp(up->lastnote->msg, "sys:", 4) == 0) {
...
static Note *mknote(char *msg, int flag) {
  Note *n;
...
int postnote(Proc *p, int dolock, char *msg, int flag) {
  Note *n;
  int ret;
...
void postnotepg(ulong noteid, char *msg, int flag) {
  Note *n;
  Proc *p;
  int i;
...
static void addbroken(void) {
  ...
  /* up->state = Broken; -- REPLACED BY FSM */
  proc_event(up, EV_BREAK);
  up->psstate = nil;
  unlock(&broken);
  sched();
}
...
void unbreak(Proc *p) {
  ...
  for (i = 0; i < broken.n; i++) {
    if (broken.p[i] == p) {
      memmove(&broken.p[i], &broken.p[i + 1],
              sizeof(Proc *) * (broken.n - (i + 1)));
...
_Noreturn void pexit(char *exitstr, int freemem) {
  ...
  if (p != nil && p->pid == up->parentpid && p->state != Broken) {
    ...
    if (exitstr != nil && exitstr[0])
      snprint(wq->w.msg, sizeof(wq->w.msg), "%s %lud: %s", up->text, up->pid,
              exitstr);
    else
      wq->w.msg[0] = '\0';
...
void dumpaproc(Proc *p) {
  ...
  if (s == nil)
    s = statename[p->state];
  print("%3lud:%10s pc %#p dbgpc %#p  %8s (%s) ut %ld st %ld bss %lux qpc %#p "
        "nl %d nd %lud lpc %#p pri %lud\n",
        p->pid, p->text, p->pc, dbgpc(p), s, statename[p->state], p->time[0],
        p->time[1], bss, p->qpc, p->nlocks, p->delaysched,
 ...
 */
static void procflushmmu(int (*match)(Proc *, void *), void *a) {
  Proc *await[MAXMACH];
  int i, nm, nwait;
  Proc *p;
...
static int matchseg(Proc *p, void *a) {
  int ns;
...
void procflushseg(Segment *s) { procflushmmu(matchseg, s); }

static int matchpseg(Proc *p, void *a) {
  Segment *s;
  int ns;
...
void procflushpseg(Physseg *ps) { procflushmmu(matchpseg, ps); }

static int matchother(Proc *p, void *a) { return p != a; }
void procflushothers(void) { procflushmmu(matchother, up); }
...
_Noreturn void error(char *err) {
  if (up == nil)
    panic("error(%s) with no user process (caller=%#p)", err,
          getcallerpc(&err));
  spllo();
...
void accounttime(void) {
  ...
  m->perf.avg_inidle =
      ((uvlong)m->perf.avg_inidle * (HZ - 1) + m->perf.inidle) / HZ;
  m->perf.inidle = 0;

  m->perf.avg_inintr =
      ((uvlong)m->perf.avg_inintr * (HZ - 1) + m->perf.inintr) / HZ;
  m->perf.inintr = 0;
 ...
 */
ulong setnoteid(Proc *p, ulong noteid) {
  Pid *i, *o;
