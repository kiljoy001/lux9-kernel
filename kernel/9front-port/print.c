...
void _assert(char *fmt) { panic("assert: %s", fmt); }

void abort(void) { panic("abort() called"); }

int readstr(ulong offset, char *buf, ulong n, char *str) {
...
int readnum(ulong offset, char *buf, ulong n, ulong val, int size) {
  char tmp[64];
  snprint(tmp, sizeof(tmp), "%*lud", size - 1, val);
  return readstr(offset, buf, n, tmp);
}

ulong strnlen(char *s, ulong max) {
  ulong i;
  for (i = 0; i < max && s[i]; i++)
    ;
  return i;
...
long clock(void) { return m->ticks; }