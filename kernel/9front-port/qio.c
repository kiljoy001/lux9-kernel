...
/*@
  @ inductive is_valid_block_list{L}(Block *b) {
  @   case nil: \forall Block *b; b == \null ==> is_valid_block_list(b);
  @   case cons: \forall Block *b; \valid(b) && is_valid_block(b) &&
  @              is_valid_block_list(b->next) ==> is_valid_block_list(b);
  @ }
  @
  @ logic integer block_list_alloc_len{L}(Block *b) =
  @   (b == \null) ? 0 : (b->lim - b->base) + block_list_alloc_len(b->next);
  @
  @ predicate is_valid_queue(Queue *q) =
  @   \valid(q) &&
  @   is_valid_block_list(q->bfirst) &&
  @   q->dlen == block_list_len(q->bfirst) &&
  @   (integer)q->wp - (integer)q->rp == block_list_alloc_len(q->bfirst);
  @*/
 ...
 */
/*@
  @ requires is_valid_block_list(b);
 ...
 */
/*@
  @ requires is_valid_block_list(bp);
  @ ensures \result == block_list_len(bp);
  @ assigns \nothing;
  @*/
...
Block *copyblock(Block *bp, int count) {
  ...
  if (bp->pool == nil || count > bp->pool->size ||
 ...
 */
/*@
  @ requires is_valid_queue(q) && \valid(tofree);
  @ ensures is_valid_queue(q);
  @ ensures is_valid_block_list(\result);
  @ ensures is_valid_block_list(*tofree);
  @ assigns q->bfirst, q->blast, q->dlen, q->rp, *tofree;
  @*/
static Block *queue_take_count(Queue *q, int len, Block **tofree) {
  Block *b, *first, **last;
  int n, m;
  ...
  while (n < len && (b = q->bfirst) != nil) {
    m = BLEN(b);
    if (m <= 0) {
      /* remove zero-length block */
      q->bfirst = b->next;
      q->rp += BALLOC(b);
      b->next = *tofree;
      *tofree = b;
...
/*@
  ...
  @   ensures is_valid_queue(q);
  @ complete behaviors;
  @ assigns q->bfirst, q->blast, q->dlen, q->rp;
  @*/
 ...
 */
static int iunlock_consumer(Queue *q) {
  int s = q->state;
 ...
 */
static int iunlock_reader(Queue *q) {
 ...
 *  called at interrupt level.
 */
static int iunlock_producer(Queue *q) {
  int s = q->state;
 ...
 *  called at process level.
 */
static int iunlock_writer(Queue *q) {
...
int qconsume(Queue *q, void *vp, int len) {
  ...
  ilock(q);
  b = queue_take_count(q, len, &tofree);
  if (b == nil) {
    q->state |= Qstarve;
    iunlock_consumer(q);
    freeblist(tofree);
...
/*@
  @ requires is_valid_queue(q) && is_valid_block_list(b) && b != \null;
  @ ensures is_valid_queue(q);
  @ assigns q->bfirst, q->blast, q->wp, q->dlen, b->next;
  @*/
static int queue_append_block(Queue *q, Block *b) {
  int len;
  Block *last = b;
...
/*@
  @ requires is_valid_queue(q) && is_valid_block_list(b) && b != \null;
  @ ensures is_valid_queue(q);
  @ ensures \result == block_list_len(b);
  @ assigns q->bfirst, q->blast, q->wp, q->dlen, b->next;
  @*/
int qaddlist(Queue *q, Block *b) {
  QDEBUG checkb(b, "qaddlist 1");
  return queue_append_block(q, b);
}
...
/*@
  @ behavior success:
  @   ensures \result != \null ==> is_valid_queue(\result);
  @   ensures \result != \null ==> \result->limit == limit;
  @ behavior failure:
  @   ensures \result == \null;
  @ complete behaviors;
  @*/
 ...
 *  head.
 */
static Block *splitblock(Block **h, int n) {
  Block *a, *b;
  int m;
...
Block *qbread(Queue *q, int len) {
  ...
  /* take at least one block, up to len bytes */
  b = queue_take_count(q, len, &tofree);
  if (b == nil) {
    /* should not happen since qwait returned success */
    iunlock_reader(q);
    qunlock(&q->rlock);
    poperror();
    freeblist(tofree);
...
/*@
  ...
  @ assigns q->bfirst, q->blast, q->dlen, q->rp, *tofree;
  @*/
static int queue_discard_bytes(Queue *q, int len, Block **tofree) {
  Block *b;
  int n, sofar;
...
int qdiscard(Queue *q, int len) {
  ...
  ilock(q);
  sofar = queue_discard_bytes(q, len, &tofree);
  iunlock_consumer(q);
...
void qflush(Queue *q) {
  ...
  ilock(q);
  queue_discard_bytes(q, q->dlen, &tofree);
  iunlock_consumer(q);
 ...
 */
int qisclosed(Queue *q) { return q->state & Qclosed; }
 ...
 *  return bytes queued
 */
int qlen(Queue *q) { return q->dlen; }

/*
 *  return true if we can read without blocking
 */
int qcanread(Queue *q) { return q->bfirst != nil; }

/*
 *  return non-zero when the queue is full
 */
int qfull(Queue *q) { return q->state & Qflow; }
...
void qsetnoblock_early(Queue *q, int onoff) { q->noblock = onoff; }
