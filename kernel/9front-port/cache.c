...
static uintptr cacheaddr(Mntcache *m, ulong pn) {
  uintptr da = pn * NFILE + (m - cache.alloc);
  return (da << PGSHIFT) | (da >> (sizeof(da) * 8 - PGSHIFT));
}

static void cnodata(Mntcache *m) { memset(m->bitmap, 0, sizeof(m->bitmap)); }

static void ctail(Mntcache *m) {
  /*@
    @ requires \valid(m);
    @ requires \valid(cache.head) && \valid(cache.tail);
...
/*@
  @ requires \valid(c);
  @ requires \valid(cache.hash + (c->qid.path % NHASH));
  @ assigns \nothing;
  @ ensures \result == \null || \valid(\result);
...
static Mntcache *clookup(Chan *c, int skipvers) {
  ...
  for (m = cache.hash[c->qid.path % NHASH]; m != nil; m = m->hash)
    if (eqchantdqid(c, m->type, m->dev, m->qid, skipvers) &&
        c->qid.type == m->qid.type)
      return m;
 ...
 */
static int cancachelock(Mntcache *m) {
...
static void cachelock(Mntcache *m) {
  if (m->locked != up) {
    qlock(m);
    assert(m->nlocked == 0);
    m->locked = up;
...
static void cacheunlock(Mntcache *m) {
  assert(m->locked == up);
  if (--m->nlocked == 0) {
    m->locked = nil;
    qunlock(m);
...
static Mntcache *ccache(Chan *c) {
  Mntcache *m;

  m = c->mcp;
  if (m != nil) {
    cachelock(m);
    if (eqchantdqid(c, m->type, m->dev, m->qid, 0) &&
        c->qid.type == m->qid.type)
      return m;
    c->mcp = nil;
    cacheunlock(m);
...
static Page *cpage(Mntcache *m, ulong pn, ulong *po, ulong *pe) {
  ulong b;
  Page *p;
...
static void cpageset(Page *p, ulong po, ulong pe) {
  assert(po < pe);
  p->va = po | (p->va & VAMASK) | ((uintptr)pe - 1) << (PGSHIFT + VABITS);
}

int cread(Chan *c, uchar *buf, int len, vlong off) {
  KMap *k;
  Page *p;
  Mntcache *m;
  int l, tot;
...
static void invalidate(Mntcache *m, ulong offset, int len) {
  ulong pn;
...
static void cachedata(Mntcache *m, uchar *buf, int len, vlong off) {
  int l;
  Page *p;
  KMap *k;
  ulong offset, pn, po, pe;
...
void cupdate(Chan *c, uchar *buf, int len, vlong off) {
  Mntcache *m;
...
void cwrite(Chan *c, uchar *buf, int len, vlong off) {
  Mntcache *m;
