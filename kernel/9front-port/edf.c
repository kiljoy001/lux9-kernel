...
/*
 * SMT: Validated by proofs/scheduler/edf_policy.v
 * Theorem: edf_utilization_bound, kernel_test_safety
 * Description: Verifies the EDF schedulability test logic used in
 * testschedulability
 */
...
static void deadlineintr(Ureg *, Timer *t) {
...
static void release(Proc *p) {
  ...
  if (e->d - now < 0) {
    ...
    DPRINT("%lud release %lud[%s], r=%lud, d=%lud, t=%lud, S=%lud\n", now,
           p->pid, statename[p->state], e->r, e->d, e->t, e->S);
    if (pt = proctrace) {
      todget(nil, &nowns);
      pt(p, SRelease, nowns);
    ...
    DPRINT("%lud release %lud[%s], too late t=%lud, called from %#p\n", now,
           p->pid, statename[p->state], e->t, getcallerpc(&p));
  }
}

static void releaseintr(Ureg *u, Timer *t) {
  Proc *p;
...
char *edfadmit(Proc *p) {
  ...
  if (r == nil) {
    ...
    if (p == up) {
      ...
      DPRINT(
          "%lud edfadmit other %lud[%s], release now: r=%lud d=%lud t=%lud\n",
          now, p->pid, statename[p->state], e->r, e->d, e->t);
      p->ta = p;
      edfunlock();
      qunlock(&edfschedlock);
...
static char *testschedulability(Proc *theproc) {
  ...
  for (steps = 0; steps < Maxsteps; steps++) {
    ...
    case Dl:
      H += p->edf->C;
      Cb = 0;
      DPRINT("\tStep %3d, Ticks %lud, pid %lud, deadline, H += %lud → %lud, Cb "
             "= %lud\n",
