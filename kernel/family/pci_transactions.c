...
static int pci_create_transaction(struct FamilyExchangePage *family,
                                  void *device_ids, int count,
                                  TransactionID *tx_id) {
  ...
  chain = xalloc_driver(sizeof(struct MultiDevicePebbleChain));
  if (!chain) {
    unlock(&tx_mgr_lock);
    return -2;
  }
...
/*@
  @ requires chain == \null || \valid(chain);@*/static void pci_cleanup_transaction(struct MultiDevicePebbleChain *chain) {
  if (!chain) {
    return;
  }
  ...
  /* Free pebble handles */
  if (chain->pebbles) {
      /*@ loop invariant 0 <= i <= chain->pebble_count;
    @ loop assigns i;
    @ loop variant chain->pebble_count - i;
    @*/
...
/*@
  @ requires chain == \null || \valid(chain);@*/static int rollback_pci_transaction(struct MultiDevicePebbleChain *chain) {
  if (!chain) {
    return -1;
  }
  ...
  /* Attempt to rollback each device */
    /*@ loop invariant 0 <= i <= chain->device_count;
    @ loop assigns i;
    @ loop variant chain->device_count - i;
    @*/
...
static int get_transaction_status(struct MultiDevicePebbleChain *chain,
                                  enum TransactionState *status,
