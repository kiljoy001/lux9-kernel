...
static int tpm_family_allocate_channel(struct FamilyExchangePage* family, void* device_id,
                                      uint32_t permissions, uint64_t* channel_id);
static int tpm_family_release_channel(struct FamilyExchangePage* family, uint64_t channel_id);
static int tpm_family_device_get_info(struct FamilyExchangePage* family, void* device,
                                     void* info_buffer, size_t* info_size);
static uint32_t tpm_family_get_capabilities(struct FamilyExchangePage* family);
static int tpm_family_begin_transaction(struct FamilyExchangePage* family, uint64_t* tx_id);
static int tpm_family_commit_transaction(struct FamilyExchangePage* family, uint64_t tx_id);
 ...
 */
static int
tpm_family_init(struct FamilyExchangePage* family)
{
    ...
    ctx = malloc(sizeof(TPMFamilyContext));
    if (ctx == nil) {
    ...
    ctx->channel_mgr.next_channel_id = 0x9000000000000000ULL;  /* TPM family channel ID prefix */
 ...
 */
static int
tpm_family_scan_devices(struct FamilyExchangePage* family)
{
 ...
 */
static int
tpm_family_allocate_channel(struct FamilyExchangePage* family, void* device_id,
                           uint32_t permissions, uint64_t* channel_id)
{
    TPMFamilyContext* ctx = family->family_specific_ctx;
    ...
    tpm_channel = malloc(sizeof(TPMChannel));
    if (tpm_channel == nil) {
        return FAMILY_ENOMEM;
    }
 ...
 */
static int
tpm_family_release_channel(struct FamilyExchangePage* family, uint64_t channel_id)
{
    TPMFamilyContext* ctx = family->family_specific_ctx;
    int i;
 ...
 */
static int
tpm_family_device_get_info(struct FamilyExchangePage* family, void* device,
                          void* info_buffer, size_t* info_size)
{
    TPMFamilyContext* ctx = family->family_specific_ctx;
    ...
    if (ctx->tpm_ctx) {
        ...
        strncpy(info->description, "TPM 2.0/1.2 Secure Element", sizeof(info->description) - 1);
    } else {
        info->hardware_backed = 0;
        strncpy(info->description, "Software Secure Element", sizeof(info->description) - 1);
    }
 ...
 */
static uint32_t
tpm_family_get_capabilities(struct FamilyExchangePage* family)
{
    TPMFamilyContext* ctx = family->family_specific_ctx;
 ...
 */
static int
tpm_family_begin_transaction(struct FamilyExchangePage* family, uint64_t* tx_id)
{
 ...
 */
static int
tpm_family_commit_transaction(struct FamilyExchangePage* family, uint64_t tx_id)
{
    return FAMILY_ETRANSACTION;
}
 ...
 */
static int
tpm_family_shutdown(struct FamilyExchangePage* family)
{
    TPMFamilyContext* ctx = family->family_specific_ctx;
    int i;
 ...
 */
int
tpm_family_register(void)
{
    int ret;
    
    print("TPM Family: Registering TPM Secure Element Family...\n");
    
    ret = family_register(FAMILY_SECURE_ELEMENT, &tpm_family_ops, "TPM Secure Element");
    if (ret != FAMILY_OK) {
...
secure_element_family_get(void)
{
 ...
 */
int
secure_element_create_channel(uint32_t permissions, uint64_t* channel_id)
{
 ...
 */
uint32_t
secure_element_get_capabilities(void)
{
    struct FamilyExchangePage* family = secure_element_family_get();
    if (family == nil) {
        return 0;
...
secure_element_init(void)
{
    ...
    print("Secure Element: Secure Element family system initialized successfully\n");
}
 ...
 */
int
secure_element_get_random(uint8_t* buffer, int len)
{
    TPMFamilyContext* ctx;
    struct FamilyExchangePage* family = secure_element_family_get();
 ...
 */
int
secure_element_attest(uint64_t channel_id, uint8_t* attestation_data, size_t* data_size)
{
    TPMFamilyContext* ctx;
    struct FamilyExchangePage* family = secure_element_family_get();
