...
/*@
  ...
  @*/
void init_resource_pool(struct FamilyExchangePage *family) {
...
/*@
  @ requires \valid(family);
  @ assigns family->event_system;
  @*/
void init_event_system(struct FamilyExchangePage *family) {
...
/*@
  @ requires \valid(family);
  @ assigns family->tx_mgr;
  @*/
void init_transaction_manager(struct FamilyExchangePage *family) {
...
int family_register(enum DeviceFamily family_type, struct FamilyOps *ops,
  ...
  struct FamilyExchangePage *family =
      xalloc_driver(sizeof(struct FamilyExchangePage));
  if (!family) {
    return -3;
  }
...
/*@
  ...
  if (family) {
    if (family->stats.active_channels > 0 &&
        family->stats.peak_channels > stats->peak_channels)
