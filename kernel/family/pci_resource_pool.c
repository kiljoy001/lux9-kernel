...
void
setup_pci_resource_pool(struct FamilyExchangePage* family)
{
    if (!family) {
        return;
    }
    ...
    struct PCIResourcePool* pool = xalloc(sizeof(struct PCIResourcePool));
    if (!pool) {
    ...
    pool->bar_resources = xalloc(sizeof(struct PCIBarResource) * pool->max_bars);
    pool->irq_resources = xalloc(sizeof(struct PCIIrqResource) * pool->max_irqs);
    pool->dma_resources = xalloc(sizeof(struct PCIDmaResource) * pool->max_dmas);
    
    if (!pool->bar_resources || !pool->irq_resources || !pool->dma_resources) {
        print("PCI: failed to allocate resource arrays\n");
        if (pool->bar_resources) xfree(pool->bar_resources);
        if (pool->irq_resources) xfree(pool->irq_resources);
        if (pool->dma_resources) xfree(pool->dma_resources);
        xfree(pool);
        return;
    }
    
    /* Initialize resource arrays */
    memset(pool->bar_resources, 0, sizeof(struct PCIBarResource) * pool->max_bars);
    memset(pool->irq_resources, 0, sizeof(struct PCIIrqResource) * pool->max_irqs);
    memset(pool->dma_resources, 0, sizeof(struct PCIDmaResource) * pool->max_dmas);
    ...
    pool->pool_white_token = pebble_create_white(PCI_FAMILY_TYPE, "pci_resource_pool", pool->total_budget);
    
    /* Attach to family context */
    ctx->resource_pool = pool;
    
    print("PCI: resource pool initialized - max_bars=%d max_irqs=%d max_dmas=%d\n",
           pool->max_bars, pool->max_irqs, pool->max_dmas);
}

/* Allocate BAR resource from pool */
int
allocate_pci_bar_resource(struct FamilyExchangePage* family, struct PCIDeviceDescriptor* device,
                         uint8_t bar_number, uint64_t channel_id, struct PCIBarResource** bar_out)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    ...
    if (bar->bound_channel) {
        ...
        bar->bar_white_token = pebble_create_white(PCI_FAMILY_TYPE, "pci_bar_access", 
                                                  bar->size / PEBBLE_GRANULARITY);
    }
    ...
    *bar_out = bar;
    ...
    print("PCI: allocated BAR%d resource id=%d addr=0x%llx size=%lld for channel %d\n",
...
int
release_pci_bar_resource(struct FamilyExchangePage* family, struct PCIBarResource* bar)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
...
int
allocate_pci_irq_resource(struct FamilyExchangePage* family, struct PCIDeviceDescriptor* device,
                         uint32_t irq_vector, uint8_t trigger_type, uint8_t polarity,
                         uint64_t channel_id, struct PCIIrqResource** irq_out)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
...
int
release_pci_irq_resource(struct FamilyExchangePage* family, struct PCIIrqResource* irq)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
...
int
allocate_pci_dma_resource(struct FamilyExchangePage* family, struct PCIDeviceDescriptor* device,
                         uint64_t size, uint32_t alignment, bool coherent,
                         uint64_t channel_id, struct PCIDmaResource** dma_out)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
    ...
    uintptr dma_phys = upamalloc(size, alignment, 0);
    if (dma_phys == 0) {
        unlock(&pool->resource_lock);
        pool->stats.allocation_failures++;
    ...
    if (dma->bound_channel) {
        int result = exchange_prepare_pci_dma(channel_id, dma->physical_address, size,
                                            dma->bound_channel->white_token,
                                            &dma->exchange_handle);
        if (result != 0) {
        ...
        dma->dma_white_token = pebble_create_white(PCI_FAMILY_TYPE, "pci_dma_access",
                                                  size / PEBBLE_GRANULARITY);
    }
    ...
    *dma_out = dma;
    ...
    print("PCI: allocated DMA resource id=%d addr=0x%llx size=%lld for channel %d\n",
...
int
release_pci_dma_resource(struct FamilyExchangePage* family, struct PCIDmaResource* dma)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
...
int
cleanup_channel_resources(struct FamilyExchangePage* family, uint64_t channel_id)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
...
void
get_pci_resource_pool_stats(struct FamilyExchangePage* family, struct ResourcePoolStats* stats)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
...
int
shutdown_pci_resource_pool(struct FamilyExchangePage* family)
{
    struct PCIFamilyContext* ctx = (struct PCIFamilyContext*)family->family_specific_ctx;
    struct PCIResourcePool* pool = ctx->resource_pool;
    ...
    xfree(pool->bar_resources);
    xfree(pool->irq_resources);
    xfree(pool->dma_resources);
