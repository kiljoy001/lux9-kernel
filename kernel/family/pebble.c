...
/*
 * SMT: Validated by proofs/pebble/pebble_security.v (Integration)
 * Description: Family integration logic relies on verified Pebble core
 * guarantees
 */
...
int family_pebble_init(struct FamilyExchangePage *family) {
  ...
  if (pebble_enabled) {
    ...
    if (ps) {
      family->family_white_token = (struct PebbleHandle *)pebble_issue_white(
          ps, nil, PEBBLE_DEFAULT_BUDGET);
 ...
 */
int family_pebble_allocate_resource(struct FamilyExchangePage *family,
                                    uint64_t channel_id, uint32_t permissions,
  ...
  *resource_handle = black_handle;
  ...
  print("Pebble-Family: Allocated pebble resource for channel 0x%llx "
        "(size=%lu)\n",
 ...
 */
int family_pebble_free_resource(struct FamilyExchangePage *family,
                                void *resource_handle) {
  FamilyPebbleState *state;
  int i;
  ...
  for (i = 0; i < MAX_FAMILY_TOKENS; i++) {
    if (state->tokens[i].active &&
        state->tokens[i].black_handle == resource_handle) {
      pebble_black_free(state->tokens[i].black_handle);
      state->tokens[i].active = 0;
 ...
 */
int family_pebble_validate_access(struct FamilyExchangePage *family,
                                  void *resource_handle,
  ...
  for (i = 0; i < MAX_FAMILY_TOKENS; i++) {
    if (state->tokens[i].active &&
        state->tokens[i].black_handle == resource_handle) {
      if ((state->tokens[i].permissions & requested_permissions) ==
