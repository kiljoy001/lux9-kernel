...
int main() {
    ...
    // Expectation: result is a structure representing 1 + 0, or just 1 if simplified.
    // For this first pass, getting a tree of (1 + 0) is acceptable.
    // SYM_ADD(SYM_CONST(1), SYM_CONST(0)))
    
    if (expr_diff->type != SYM_ADD) {
        printf("FAILED: Derivative should be an ADD expression (d/dx(x) + d/dx(5))\n");
        return 1;
    }
    
    sym_expr_t* left = expr_diff->data.op.left;
    sym_expr_t* right = expr_diff->data.op.right;
