...
typedef enum {
  /* Atoms */
  SYM_VAR,      // Variable: x, y, z
  SYM_CONST,    // Integer constant: 0, 1, 5
  SYM_RATIONAL, // Rational: numerator/denominator
...
typedef struct sym_expr {
  ...
  union {
    ...
    struct {       // For SYM_RATIONAL
      int64_t num;
      int64_t den;
    } rational;
    struct sym_expr *unary; // For SYM_NEG, SYM_SIN, etc.
    struct {
...
typedef struct vm_frame {
  ...
  vm_generic_context_t
      generic_context; // Context for generic parameters (!0, !!0)
} vm_frame_t;

// VM Execution State
typedef struct {
  vm_frame_t *current_frame; // Current execution frame
  vm_stack_t *eval_stack;    // Evaluation stack (Linked List Mode)

  /* Flat Stack Mode (Zero Alloc) */
  bool use_flat_stack;
  vm_value_t *flat_stack_base;
  uint32_t flat_stack_capacity;
  uint32_t flat_stack_top; // Index of next free slot
  ...
  void *
      assembly; // Pointer to il_assembly_t (void* to avoid circular dependency)
...
bool vm_symbolic_expr(vm_value_t *left, vm_value_t *right,
                      vm_value_t *result); // Deprecated generic
...
// Generics Support
clr_runtime_type_t *vm_parse_type_signature(void *assembly,
                                            const uint8_t **sig_ptr,
                                            vm_generic_context_t *context);
clr_runtime_type_t *vm_resolve_type_token(void *assembly, uint32_t token,
                                          vm_generic_context_t *context);
