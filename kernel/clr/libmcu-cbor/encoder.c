...
static cbor_error_t encode_core(cbor_writer_t *writer, uint8_t major_type,
                                uint8_t const *data, uint64_t datasize,
                                bool indefinite) {
...
cbor_error_t cbor_encode_unsigned_integer(cbor_writer_t *writer,
                                          uint64_t value) {
  return encode_core(writer, 0, NULL, value, false);
}

cbor_error_t cbor_encode_negative_integer(cbor_writer_t *writer,
                                          int64_t value) {
  return encode_core(writer, 1, NULL, ((uint64_t)-value) - 1, false);
}

cbor_error_t cbor_encode_byte_string(cbor_writer_t *writer, uint8_t const *data,
                                     size_t datasize) {
...
cbor_error_t cbor_encode_text_string(cbor_writer_t *writer, char const *text,
                                     size_t textsize) {
  return encode_core(writer, 3, (uint8_t const *)text, textsize, false);
}
