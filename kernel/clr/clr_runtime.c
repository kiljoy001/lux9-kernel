...
void *clr_string_from_literal(u32int us_index) {
    if (!current_assembly) return nil;
 ...
 */
static void *clr_compile_method_to_wasm(il_assembly_t *assembly, il_method_t *method, ulong *out_size) {
    ...
    if (out_size) *out_size = wasm_res.wasm_size;
...
static il_method_t *clr_find_entry_point(il_assembly_t *assembly) {
  ...
  if (assembly->cli_header.entry_point_token) {
    return il_get_method_by_token(assembly, assembly->cli_header.entry_point_token);
  }
  return nil;
}
...
int clr_execute_assembly(void *dll_data, ulong dll_size) {
    ...
    il_assembly_t *assembly = il_parse_assembly_memory((u8int *)dll_data, dll_size, &err);
    if (!assembly) {
    ...
    if (!env) { print("m3_NewEnvironment failed\n"); return -1; }
    
    IM3Runtime runtime = m3_NewRuntime(env, 64*1024, nil);
    if (!runtime) { print("m3_NewRuntime failed\n"); return -1; }
    
    IM3Module module;
    M3Result result = m3_ParseModule(env, &module, wasm_bytes, wasm_size);
    if (result) { print("m3_ParseModule: %s\n", result); return -1; }
    
    result = m3_LoadModule(runtime, module);
    if (result) { print("m3_LoadModule: %s\n", result); return -1; }
    
    /* Link Host Functions */
    result = lux9_link_wasi(module);
    if (result) { print("lux9_link_wasi: %s\n", result); return -1; }
    
    /* Run Main */
    IM3Function f;
    result = m3_FindFunction(&f, runtime, "Main");
    if (result) { print("m3_FindFunction: %s\n", result); return -1; }
