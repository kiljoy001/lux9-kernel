...
void fruity_basic_block_add_successor(fruity_basic_block_t *block,
  ...
  if (block->successor_count >= block->successor_capacity) {
    ulong new_cap =
        block->successor_capacity == 0 ? 2 : block->successor_capacity * 2;
    fruity_basic_block_t **new_arr =
        xalloc(new_cap * sizeof(fruity_basic_block_t *));
    if (block->successors != nil) {
...
void fruity_basic_block_add_predecessor(fruity_basic_block_t *block,
  ...
  if (block->predecessor_count >= block->predecessor_capacity) {
    ulong new_cap =
        block->predecessor_capacity == 0 ? 2 : block->predecessor_capacity * 2;
    fruity_basic_block_t **new_arr =
        xalloc(new_cap * sizeof(fruity_basic_block_t *));
    if (block->predecessors != nil) {
...
int fruity_module_verify(fruity_module_t *module) {
  if (module == nil)
    return -1;
  if (module->name == nil || module->name[0] == '\0')
    return -1;
...
int fruity_function_verify(fruity_function_t *func) {
  if (func == nil)
    return -1;
  if (func->name == nil || func->name[0] == '\0')
    return -1;
  if (func->entry_block == nil && func->block_count > 0)
    return -1; /* Has blocks but no entry */
...
int fruity_basic_block_verify(fruity_basic_block_t *block) {
  ...
  /* Verify block has at least one instruction (or is empty entry) */
  if (block->instruction_count == 0)
    return 0; /* Empty blocks are valid */

  /* Verify last instruction is a terminator (if block has instructions) */
  if (block->instructions_tail != nil) {
    if (!fruity_opcode_is_terminator(block->instructions_tail->opcode)) {
      /* Non-terminator at end - implicit fallthrough is valid */
    }
  }
  return 0;
}
