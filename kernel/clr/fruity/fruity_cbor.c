...
ulong fruity_module_to_cbor(fruity_module_t *module, u8int *buf, ulong bufsize,
  ...
  if (module == nil || buf == nil) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "null parameters");
    return 0;
  }
  ...
  if (module->name)
    cbor_encode_text_string(&writer, module->name,
                            (size_t)strlen(module->name));
  else
  ...
  for (func = module->functions_head; func != nil; func = func->next) {
    ...
    cbor_encode_text_string(&writer, "name", 4);
    if (func->name)
      cbor_encode_text_string(&writer, func->name, (size_t)strlen(func->name));
    else
...
fruity_module_t *fruity_module_from_cbor(u8int *data, ulong datalen,
  ...
  if (data == nil) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "null data");
    return nil;
  }
  ...
  if (err != CBOR_SUCCESS) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "CBOR parse error: %d", err);
    return nil;
  }
  ...
  if (module == nil) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "allocation failed");
    return nil;
  }
  ...
  if (nitems < 1 || items[0].type != CBOR_ITEM_MAP) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "root must be map");
    free(module);
    return nil;
  }
  ...
  if (!found_name || !found_version || !found_functions) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "missing required fields");
    free(module);
    return nil;
  }
  ...
  if (cbor_decode(&reader, &items[name_idx], name_buf, sizeof(name_buf) - 1) !=
      CBOR_SUCCESS) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "failed to decode name");
    free(module);
    return nil;
  }
  ...
  /* Extract version */
  u64int version_val;
  if (cbor_decode(&reader, &items[version_idx], &version_val,
                  sizeof(version_val)) != CBOR_SUCCESS) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "failed to decode version");
    free(module->name);
    free(module);
    return nil;
  }
  ...
  if (items[functions_idx].type != CBOR_ITEM_ARRAY) {
    if (errbuf && errbuf_size > 0)
      snprint(errbuf, (int)errbuf_size, "functions must be array");
    free(module->name);
    free(module);
    return nil;
  }
  ...
  for (ulong fi = 0; fi < func_count && idx < nitems; fi++) {
    ...
    if (func_map->type != CBOR_ITEM_MAP) {
      if (errbuf && errbuf_size > 0)
        snprint(errbuf, (int)errbuf_size, "function must be map");
    ...
    if (func == nil) {
      if (errbuf && errbuf_size > 0)
        snprint(errbuf, (int)errbuf_size, "function allocation failed");
    ...
    if (found_func_name) {
      ...
      cbor_decode(&reader, &items[func_name_idx], func_name_buf,
                  sizeof(func_name_buf) - 1);
      if (func->name)
        free(func->name);
      ...
      for (ulong bi = 0; bi < block_count && block_idx < nitems; bi++) {
        ...
        if (found_bb_instrs && items[bb_instrs_idx].type == CBOR_ITEM_ARRAY) {
          ...
          for (ulong ii = 0; ii < instr_count && instr_idx < nitems; ii++) {
            ...
            for (ulong ip = 0; ip < instr_pairs && instr_map_idx + 1 < nitems;
                 ip++) {
            ...
            if (found_opcode) {
              u64int opcode_val;
              cbor_decode(&reader, &items[opcode_idx], &opcode_val,
                          sizeof(opcode_val));

              fruity_instruction_t *instr =
                  fruity_instruction_create((fruity_opcode_t)opcode_val);
              if (instr != nil) {
