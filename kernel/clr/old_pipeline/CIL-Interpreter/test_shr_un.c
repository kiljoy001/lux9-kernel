#include <stdio.h>
#include <stdlib.h>
#include "include/execution_engine.h"

int main() {
    printf("=== Testing SHR_UN Operation Execution ===\n");
    
    vm_init_t init = {0};
    vm_execution_state_t* state = vm_create_execution_state(&init);
    
    // Create bytecode: LDC_U4 16, LDC_U4 2, SHR_UN
    uint8_t bytecode[] = {
        0x22, 0x10, 0x00, 0x00, 0x00,  // LDC_U4 16
        0x22, 0x02, 0x00, 0x00, 0x00,  // LDC_U4 2
        0x64,                          // SHR_UN
    };
    
    printf("Executing: 16 LDC_U4, 2 LDC_U4, SHR_UN\n");
    printf("Expected: 16 >> 2 = 4 (unsigned)\n");
    
    bool result = vm_execute_method(state, bytecode, sizeof(bytecode), 0);
    
    if (result) {
        printf("Execution: SUCCESS\n");
        
        vm_value_t top;
        if (vm_stack_pop(state, &top)) {
            printf("Result on stack: %u (type %d)\n", top.value.u4, top.type);
            if (top.value.u4 == 4) {
                printf("✓ SHR_UN operation works correctly!\n");
            } else {
                printf("✗ SHR_UN operation result incorrect (expected 4, got %u)\n", top.value.u4);
            }
        } else {
            printf("✗ Failed to pop result from stack\n");
        }
    } else {
        printf("Execution: FAILED\n");
        const char* error = vm_get_error(state);
        if (error) {
            printf("Error: %s\n", error);
        }
    }
    
    vm_destroy_execution_state(state);
    return 0;
}
