...
bool is_extended_opcode(uint8_t opcode) { return opcode == 0xFE; }
...
static int64_t read_int64(const uint8_t *data, size_t offset) {
  uint32_t low =
      (uint32_t)(data[offset] | (data[offset + 1] << 8) |
                 (data[offset + 2] << 16) | (data[offset + 3] << 24));
  uint32_t high =
      (uint32_t)(data[offset + 4] | (data[offset + 5] << 8) |
                 (data[offset + 6] << 16) | (data[offset + 7] << 24));
  return ((int64_t)high << 32) | low;
}

// Helper function to read a 32-bit unsigned integer from bytecode
// (little-endian)
...
// Function to decode instruction into a user-provided buffer (Zero-Alloc)
bool decode_instruction_to_buffer(cil_decoder_t *decoder,
                                  cil_instruction_t *instruction) {
  if (decoder == NULL || decoder->has_error ||
      decoder->offset >= decoder->bytecode_size || instruction == NULL) {
    return false;
  }
  ...
  if (is_extended_opcode(opcode_byte)) {
    ...
    instruction->opcode =
        (cil_opcode_t)(0x100 +
                       extended_opcode); // Extended opcodes start at 0x100
    instruction->size = 2;
    decoder->offset += 2;
  } else {
  ...
  case CIL_OPERAND_SHORT:
    ...
    instruction->operand.short_val =
        (int16_t)read_int32(decoder->bytecode, decoder->offset);
    instruction->size += 2;
    decoder->offset += 2;
    break;
  ...
  case CIL_OPERAND_BRANCH:
    ...
    instruction->operand.int_val =
        read_int32(decoder->bytecode, decoder->offset);
    instruction->size += 4;
    decoder->offset += 4;
    break;
  ...
  case CIL_OPERAND_LONG:
    ...
    instruction->operand.long_val =
        read_int64(decoder->bytecode, decoder->offset);
    instruction->size += 8;
    decoder->offset += 8;
    break;
  ...
  case CIL_OPERAND_FLOAT:
    ...
    instruction->operand.token =
        read_uint32(decoder->bytecode, decoder->offset);
    instruction->size += 4;
    decoder->offset += 4;
    break;
  ...
  case CIL_OPERAND_BRANCH_SHORT:
    ...
    instruction->operand.branch_offset_short =
        decoder->bytecode[decoder->offset];
    instruction->size += 1;
    decoder->offset += 1;
    break;
  ...
  case CIL_OPERAND_SWITCH: {
    ...
    /* NOTE: Using malloc here violates strict zero-alloc, but SWITCH is rare.
       For true zero-alloc, we should return a pointer to the table in bytecode.
     */
    instruction->operand.switch_table.targets =
        (int32_t *)malloc(num_targets * sizeof(int32_t));
    ...
    for (uint32_t i = 0; i < num_targets; i++) {
      ...
      instruction->operand.switch_table.targets[i] =
          read_int32(decoder->bytecode, decoder->offset);
      instruction->size += 4;
      decoder->offset += 4;
    }
...
// Function to allocate and decode
cil_instruction_t *decode_next_instruction(cil_decoder_t *decoder) {
  if (decoder == NULL)
    return NULL;

  cil_instruction_t *instruction =
      (cil_instruction_t *)malloc(sizeof(cil_instruction_t));
  if (instruction == NULL) {
    decoder->has_error = true;
    decoder->error_message = "Memory allocation failed";
    return NULL;
  ...
  if (!decode_instruction_to_buffer(decoder, instruction)) {
    free(instruction);
    return NULL;
  }
