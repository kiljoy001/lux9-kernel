...
/* Check EOF */
int exchange_feof(ExchangeFILE *fp) { return fp ? fp->eof : 1; }

/* Check error */
int exchange_ferror(ExchangeFILE *fp) { return fp ? fp->error : 1; }
...
/* Block write */
size_t exchange_fwrite(const void *ptr, size_t size, size_t nmemb,
                       ExchangeFILE *fp) {
  size_t total;
...
/* Tell position */
long exchange_ftell(ExchangeFILE *fp) { return fp ? (long)fp->pos : -1L; }
    ...
    if (*f == '%') {
      ...
      if (*f == 'd' || *f == 'i') {
        ...
        if (fp->data[fp->pos] == '-' || fp->data[fp->pos] == '+')
          *p++ = fp->data[fp->pos++];
        while (fp->pos < fp->len && fp->data[fp->pos] >= '0' &&
               fp->data[fp->pos] <= '9')
          *p++ = fp->data[fp->pos++];
        *p = '\0';
        *ip = atoi(buf);
...
int exchange_vfprintf(ExchangeFILE *fp, const char *fmt, va_list ap) {
  ...
  len = vsnprint(buf, sizeof(buf), (char *)fmt, ap);
  if (len < 0)
    return len;
