...
pebble_selftest(void)
{
	PebbleState *ps;
	PebbleWhite *white;
	UserCapability *black_cap; // Move to heap
	PebbleBlue *blue;
	PebbleRed *red;
	...
	/* Allocate black_cap on heap to avoid stack issues near Proc struct */
	black_cap = mallocz(sizeof(UserCapability), 1);
	if(black_cap == nil)
		return;
