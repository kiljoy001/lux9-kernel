 ...
 * Adapted for Lux9 kernel by removing Linux-specific dependencies.
 *
...
typedef struct {
	u64int key[2];  /* On 64-bit, HalfSipHash is actually SipHash for performance */
} hsiphash_key_t;

/* Check if key is zero */
static inline int
siphash_key_is_zero(const siphash_key_t *key)
{
