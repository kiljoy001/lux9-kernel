...
#ifndef BLIND_LEDGER_H
#define BLIND_LEDGER_H

#include "dat.h"

// Define sizes for cryptographic hashes and secrets
...
typedef struct BlindLedgerEntry {
    ...
    u8int secret[BLIND_LEDGER_SECRET_SIZE]; /* Cryptographic secret for capability derivation */
    ...
    BlindLedgerHash process_hash; /* Mutable hash of dynamic properties (owner, perms, state) */
} BlindLedgerEntry;
...
BlindLedgerError    ledger_mint(UserCapability *out_cap, uintptr pa, ulong len, Proc *owner, u32int permissions, const u8int *vault_secret);
BlindLedgerError    ledger_verify(const UserCapability *cap, BlindLedgerEntry *out_entry);
BlindLedgerError    ledger_transfer(const UserCapability *cap, Proc *from_owner, Proc *to_owner);
BlindLedgerError    ledger_burn(const UserCapability *cap, Proc *owner);
BlindLedgerError    ledger_lookup_by_pa_and_owner(uintptr pa, Proc *owner, UserCapability *out_cap, BlindLedgerEntry *out_entry);
...
typedef struct LedgerRollbackToken {
    UserCapability original_capability; // Original capability hash before transfer
    Proc *original_owner;               // Original owner before transfer
    BlindLedgerHash original_process_hash; // Original process_hash before transfer
    u64int is_valid;                    // Magic value to verify token validity
} LedgerRollbackToken;

#define ROLLBACK_TOKEN_MAGIC 0x524F4C4C4241434BULL // "ROLLBACK" in hex

BlindLedgerError    ledger_transfer_reversible(const UserCapability *cap, Proc *from_owner, Proc *to_owner, LedgerRollbackToken *rollback_token);
BlindLedgerError    ledger_rollback_transfer(const UserCapability *current_cap, LedgerRollbackToken *rollback_token);
