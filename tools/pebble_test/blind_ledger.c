...
// Derivation counter for unique child hashes
...
static int ledger_tree_insert(LedgerEntryNode *new_node) {
  ...
  rb_insert_augmented(&new_node->rb, &ledger_tree, ledger_augment_rotate, nil);
...
ledger_transfer_reversible(const UserCapability *cap, Proc *from_owner,
  ...
  rb_erase_augmented(&node->rb, &ledger_tree, ledger_augment_rotate, nil);
...
BlindLedgerError ledger_burn(const UserCapability *cap, Proc *owner) {
  ...
  // Use augmented erase
  struct rb_node *parent = rb_parent(&node->rb); // Parent for propagation
  rb_erase_augmented(&node->rb, &ledger_tree, ledger_augment_rotate, nil);
  ...
  // assume `node`'s parent is a reasonable start place if valid.
...
static u8int derivation_key[32] = {0};
...
BlindLedgerError ledger_derive(const UserCapability *parent_cap, Proc *owner,
  ...
  // Copy entry fields from parent (but NOT the capability hash yet)
  child_node->entry.physical_address = parent_node->entry.physical_address;
  child_node->entry.span_len = parent_node->entry.span_len;
  child_node->entry.owner = parent_node->entry.owner;
  memmove(child_node->entry.secret, parent_node->entry.secret,
          BLIND_LEDGER_SECRET_SIZE);
  memmove(child_node->entry.leaf_hash, parent_node->entry.leaf_hash,
          BLIND_LEDGER_CAP_SIZE);
  memmove(child_node->entry.process_hash, parent_node->entry.process_hash,
          BLIND_LEDGER_CAP_SIZE);
  child_node->entry.permissions = child_constraints;
  child_node->entry.state = BLIND_LEDGER_STATE_ACTIVE;
  child_node->entry.epoch = global_epoch;
  memmove(child_node->entry.parent_hash, parent_cap->hash,
          BLIND_LEDGER_CAP_SIZE);

  // Generate NEW capability hash for child (must be unique)
  // Include derivation_counter to ensure uniqueness even with same epoch
  ...
  // Set remaining capability fields
  child_node->entry.capability.size = parent_node->entry.capability.size;
  child_node->entry.capability.type = parent_node->entry.capability.type;
  ...
  // RB-tree node is zeroed by mallocz, ready for insert
...
BlindLedgerError ledger_verify_derivation_proof(const DerivationProof *proof) {
  ...
  for (u32int i = 0; i < proof->chain_length; i++) {
    ...
    compute_derivation_sig(step->parent_hash, step->constraints, current_hash,
                           expected_sig);
    if (memcmp(step->derivation_sig, expected_sig, BLIND_LEDGER_CAP_SIZE) != 0)
      return BLIND_LEDGER_EPERM;
