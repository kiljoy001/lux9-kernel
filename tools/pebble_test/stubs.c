...
error(char *s)
{
    ...
    if(up) strncpy(up->errstr, s, ERRMAX-1);
    if(error_active)
        longjmp(error_buf, 1);
    else {
...
malloc(ulong size)
{
    return mallocz(size, 0);
}

void*
mallocz(ulong size, int clear)
{
...
void*
xallocz(ulong size, int clear)
{
    return mallocz(size, clear);
}
...
int tpm_get_random(u8int *buffer, int len) {
    for(int i=0; i<len; i++) buffer[i] = rand();
    return len;
}
...
void crypto_hmac_sha256(u8int *out, u8int *key, ulong klen, u8int *data, ulong dlen) {
    memset(out, 0xBB, 32);
}

/* SipHash Mock */
/* Must match siphash.h signatures if linked, but here we compile stub logic or use copy?
   We copied siphash.h. But we construct stub implementations here.
   If siphash.h declares functions, we must match.
*/
