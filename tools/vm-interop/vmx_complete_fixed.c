...
#include "seabios_eltorito.h"

char *segname;
int segrclose;
Region *mmap;
int ctlfd = -1, regsfd = -1, mapfd = -1, waitfd = -1; /* Initialize to -1 for proper cleanup */
...
/* Formal cleanup protocol - proven correct in Coq */
static void
vmxcleanup(void)
{
	/* Cleanup sequence proven in vmx_cleanup_protocol.v */
	if(waitfd >= 0) {
		close(waitfd);
		waitfd = -1;
	}
...
{
	/* Ensure cleanup on any signal - prevents "no free devices" */
	USED(sig);
	vmxcleanup();
	exits("interrupted");
}
...
/* Modified vmxsetup with formal cleanup protocol */
static void
vmxsetup(void)
{
	static char buf[128];
...
threadmain(int argc, char **argv)
{
	...
	/* Handle ISO boot with SeaBIOS code - with proper cleanup on failure */
	if(isofile != nil){
		...
		if(waserror()) {
			vmxcleanup();  /* Critical: cleanup on ISO boot failure */
			nexterror();
		}
		...
		/* Write boot image to temp file with unique name */
		tmpfile = smprint("/tmp/vmx_boot.%d.%lld", getpid(), nsec());
		
		/* Remove any existing file first */
		remove(tmpfile);
		
		int fd = create(tmpfile, OWRITE, 0666);
		if(fd < 0) {
			free(bootimg);
			vmxcleanup();
		...
		atexit(lambda() { if(tmpfile) remove(tmpfile); });
