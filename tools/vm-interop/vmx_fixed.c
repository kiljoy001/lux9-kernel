...
extern int extract_alpine_kernel(char*, void**, long*);

/* Linux boot protocol */
extern int is_linux_kernel(void*, long);
extern uintptr setup_linux_boot(void*, long, char*, void*, long);
extern long get_linux_setup_size(void*);
extern void* get_linux_kernel32(void*);
extern long get_linux_kernel32_size(void*, long);
extern int create_linux_loader(char*, void*, long, uintptr);
...
threadmain(int argc, char **argv)
{
	...
	if(isofile != nil){
		...
		/* Extract Alpine Linux kernel directly */
		ret = extract_alpine_kernel(isofile, &bootimg, &bootsize);
		if(ret != 0)
			sysfatal("Alpine kernel extraction failed: error %d", ret);

		print("Alpine kernel: extracted %ld bytes\n", bootsize);

		/* Check if it's a Linux kernel */
		if(is_linux_kernel(bootimg, bootsize)){
			print("Linux kernel detected, boot protocol version %x.%02x\n",
			      is_linux_kernel(bootimg, bootsize) >> 8,
			      is_linux_kernel(bootimg, bootsize) & 0xFF);

			/* Write complete kernel image to temp file for loadkernel() */
			/* VMX's loadkernel() will detect it's Linux and use trylinux() */
			char *kernelfile = malloc(256);
			if(kernelfile == nil)
				sysfatal("cannot allocate memory for kernel filename");
			snprint(kernelfile, 256, "/tmp/vmx_kernel.%d", getpid());
			int fd = create(kernelfile, OWRITE|ORCLOSE, 0600);
			if(fd < 0 || write(fd, bootimg, bootsize) != bootsize)
				sysfatal("cannot write kernel: %r");
			close(fd);

			free(bootimg);

			/* Pass complete kernel to loadkernel() which will detect Linux */
			argv[0] = kernelfile;
		} else {
			/* Not Linux, write as-is */
			char tmpfile[256];
